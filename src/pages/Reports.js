...
function ReportAccordionItem({ report }) {
  ...
  return (
    <Card className="bg-[#0d0d0d] border-[#2a2a2a] overflow-hidden">
      ...
      >
        <div className="flex items-center space-x-4">
          <div className="w-10 h-10 bg-[#00c896]/20 rounded-sm flex items-center justify-center shrink-0">
        ...
        <ChevronDown
          className={`w-5 h-5 text-[#888888] shrink-0 ml-4 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`}
        />
      </button>
        ...
        <div className="px-5 pb-5 border-t border-[#2a2a2a]">
          ...
          <div className="flex justify-end pt-3 mb-3">
            <Button
              variant="outline"
              size="sm"
              className="border-[#2a2a2a] hover:bg-[#1a1a1a] text-xs"
              onClick={async () => {
                try {
          ...
          {/* Period data */}
          <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-sm p-4 mb-4">
            <h4 className="font-heading text-sm text-[#888888] uppercase tracking-wider mb-3">Dados do Período</h4>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                <p className="text-xs text-[#888888] mb-1">Tarefas</p>
                <p className="font-data text-lg">{report.data?.tasks_completed ?? 0}/{report.data?.tasks ?? 0}</p>
              </div>
              <div>
                <p className="text-xs text-[#888888] mb-1">Hábitos</p>
                <p className="font-data text-lg">{report.data?.total_habits_completions ?? 0}</p>
              </div>
              <div>
                <p className="text-xs text-[#888888] mb-1">Receitas</p>
                <p className="font-data text-lg text-[#00c896]">R$ {(report.data?.income ?? 0).toFixed(2)}</p>
              </div>
              <div>
                <p className="text-xs text-[#888888] mb-1">Despesas</p>
          ...
          {/* Insights */}
          <div>
            <h4 className="font-heading text-sm text-[#888888] uppercase tracking-wider mb-3 flex items-center">
      ...
      <div className="flex-1 ml-0 md:ml-64 p-4 md:p-8 pb-24 md:pb-8 pt-[72px] md:pt-8">
        <div className="max-w-6xl mx-auto">
          <div className="mb-8">
            ...
            <p className="text-[#888888]">Análises e insights com IA</p>
          </div>
                ...
                <div className="flex items-center space-x-4">
                  <div className="flex-1">
                    <Select value={reportType} onValueChange={setReportType}>
                      <SelectTrigger data-testid="report-type-select" className="bg-[#1a1a1a] border-[#2a2a2a] text-white">
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <Button
                    ...
                    disabled={loading}
                    className="bg-[#00c896] hover:bg-[#0062CC] uppercase text-xs tracking-widest"
                  >
          ...
          <div className="space-y-4">
            {reports.length === 0 ? (
              <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-8 text-center">
                ...
                <p className="text-[#888888]">Nenhum relatório gerado ainda</p>
              </Card>
            ) : (
              reports.map((report) => (
