      ...
      <div className="flex min-h-screen bg-[#050505]">
        ...
        <div className="flex-1 p-4 md:p-8 md:ml-72">
          <div className="mb-8 pt-12 md:pt-0">
            <div className="h-8 bg-[#2a2a2a] rounded w-64 animate-pulse mb-4" />
          ...
          <div className="mb-6 md:mb-8 pt-12 md:pt-0">
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div>
                ...
                <p className="text-[#888888] text-sm md:text-base">Visão geral das operações</p>
              </div>
              {/* Global Search */}
              <div className="relative w-full md:w-80">
                ...
                <Input 
                  ...
                  className="bg-[#0d0d0d] border-[#2a2a2a] text-white pl-10 pr-8"
                />
                {searchQuery && (
                ...
                {showSearch && searchResults.length > 0 && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-[#0d0d0d] border border-[#2a2a2a] rounded-lg shadow-2xl z-50 max-h-80 overflow-y-auto">
                    {searchResults.map((r, i) => (
                      <button
                        key={i}
                        onClick={() => { navigate(r.link); setShowSearch(false); setSearchQuery(""); }}
                        className="w-full flex items-center gap-3 p-3 hover:bg-[#1a1a1a] transition-colors text-left border-b border-[#2a2a2a] last:border-0"
                      >
                        <span className="text-lg">{r.icon}</span>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-white truncate">{r.title}</p>
                          <p className="text-xs text-[#555555]">{r.subtitle}</p>
                        </div>
            ...
            <div className="mb-6 space-y-2">
              {reminders.slice(0, 3).map((r, idx) => (
                <div key={idx} className={`flex items-center gap-3 p-3 rounded-lg border ${
                  r.priority === 'high' ? 'bg-red-500/5 border-red-900/50' : 'bg-[#0d0d0d] border-[#2a2a2a]'
                }`}>
                  <span className="text-lg">{r.icon}</span>
                  <p className="text-sm text-[#888888] flex-1">{r.message}</p>
                  <button onClick={() => navigate(r.action_link)} className="text-xs text-[#00c896] hover:underline whitespace-nowrap">
                    Ver
                  </button>
                  <button onClick={() => dismissReminder(idx)} className="text-[#555555] hover:text-white">
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ))}
            ...
            <div className="mb-6">
              <div className="flex gap-3 overflow-x-auto pb-2">
                {crossSuggestions.map((s, idx) => (
                  <Card key={idx} className="bg-gradient-to-r from-[#0d0d0d] to-[#0a0a1a] border-[#2a2a2a] p-4 min-w-[280px] flex-shrink-0">
                    <div className="flex items-start gap-3">
                      <span className="text-2xl">{s.icon}</span>
                      <div className="flex-1">
                        <p className="text-sm font-medium text-white">{s.title}</p>
                        <p className="text-xs text-[#888888] mt-1">{s.message}</p>
                        <button onClick={() => navigate(s.action_link)} className="text-xs text-[#00c896] hover:underline mt-2 flex items-center gap-1">
            ...
            <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6 md:mb-8">
                ...
                {dailySummary && dailySummary.summary && (
                  <Card className="bg-gradient-to-br from-[#0d0d0d] to-[#1a1a1a] border-[#2a2a2a] p-4 md:p-5 relative overflow-hidden">
                    <div className="absolute top-0 right-0 w-40 h-40 bg-[#00c896]/5 rounded-full blur-[60px] pointer-events-none" />
                    <div className="relative z-10">
                      <div className="flex items-center gap-2 mb-3">
                        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#00c896] to-[#A855F7] flex items-center justify-center">
                          <Sparkles className="w-4 h-4 text-white" />
                        </div>
                        <div>
                          <h3 className="font-heading text-sm">BRIEFING DIÁRIO</h3>
                          <p className="text-[10px] text-[#555555]">Gerado por IA</p>
                        </div>
                        {dailySummary.summary.score !== undefined && (
                          <div className="ml-auto flex items-center gap-1.5">
                            <div className="w-10 h-10 rounded-full border-2 flex items-center justify-center" style={{ borderColor: dailySummary.summary.score >= 70 ? '#00c896' : dailySummary.summary.score >= 40 ? '#FF9500' : '#FF3B30' }}>
                              <span className="font-data text-xs" style={{ color: dailySummary.summary.score >= 70 ? '#00c896' : dailySummary.summary.score >= 40 ? '#FF9500' : '#FF3B30' }}>{dailySummary.summary.score}</span>
                            </div>
                          </div>
                        )}
                      </div>
                      <p className="text-sm text-[#D4D4D8] mb-2">{dailySummary.summary.greeting}</p>
                      <p className="text-xs text-[#888888] mb-3">{dailySummary.summary.progress_summary}</p>
                      {dailySummary.summary.pending_items && dailySummary.summary.pending_items.length > 0 && (
                        <div className="mb-3">
                          <p className="text-[10px] text-[#555555] uppercase tracking-wider mb-1.5">Pendente</p>
                          <div className="flex flex-wrap gap-1.5">
                            {dailySummary.summary.pending_items.slice(0, 5).map((item, i) => (
                              <span key={i} className="text-[10px] bg-[#1A1A1A] text-[#888888] px-2 py-0.5 rounded-full">{item}</span>
                            ))}
                          </div>
                        </div>
                      )}
                      {dailySummary.summary.priority_action && (
                        <div className="bg-[#00c896]/10 border border-[#00c896]/20 rounded-lg px-3 py-2">
                          <p className="text-[10px] text-[#00c896] uppercase tracking-wider mb-0.5">Ação Prioritária</p>
                          <p className="text-xs text-white">{dailySummary.summary.priority_action}</p>
                        </div>
                      )}
                      {dailySummary.summary.motivation && (
                        <p className="text-[10px] text-[#555555] italic mt-2">"{dailySummary.summary.motivation}"</p>
                      )}
                    </div>
                  </Card>
                )}
                ...
                {globalStreaks && (
                  <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4 md:p-5">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="text-right">
                          ...
                          <p className="text-[10px] text-[#555555] uppercase">Dias</p>
                        </div>
                        {globalStreaks.longest_streak > 0 && (
                          <div className="text-right border-l border-[#2a2a2a] pl-3">
                            ...
                            <p className="text-[10px] text-[#555555] uppercase">Recorde</p>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex gap-1.5 mb-4">
                      {(globalStreaks.heatmap || []).map((day, i) => (
                        <div key={i} className="flex-1 text-center">
                          <div className={"w-full aspect-square rounded-md flex items-center justify-center text-[10px] font-data transition-colors " + (day.active ? "text-white" : "text-[#333333]")} style={{ backgroundColor: day.active ? (day.count >= 3 ? '#FF9500' : day.count >= 2 ? '#FF9500AA' : '#FF950066') : '#1A1A1A' }}>
                            {day.count || '-'}
                          </div>
                          <p className="text-[8px] text-[#333333] mt-1">{day.date.slice(8)}</p>
                        </div>
                      ))}
                    </div>
                    {/* Module Streaks */}
                    <div className="grid grid-cols-5 gap-1.5">
                      {[
                        { key: "tasks", label: "Tarefas", icon: CheckSquare, color: "#00c896" },
                        { key: "habits", label: "Hábitos", icon: TrendingUp, color: "#00c896" },
                        ...
                        return (
                          <div key={m.key} className="text-center bg-[#1a1a1a] rounded-lg p-2">
                            <ModIcon className="w-3.5 h-3.5 mx-auto mb-1" style={{ color: streak > 0 ? m.color : '#333333' }} />
                            <p className="font-data text-sm" style={{ color: streak > 0 ? m.color : '#333333' }}>{streak}</p>
                            <p className="text-[7px] text-[#333333] uppercase">{m.label}</p>
                          </div>
                        );
                      })}
                    </div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-6 md:mb-8">
                <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4 md:p-6">
                  <div className="flex items-center justify-between mb-2 md:mb-4">
                    <CheckSquare className="w-6 h-6 md:w-8 md:h-8 text-[#00c896]" />
                    <span className="font-data text-lg md:text-2xl">{stats.tasks_completed_today ?? 0}/{stats.tasks_today ?? 0}</span>
                  </div>
                  <p className="text-[#888888] uppercase text-[10px] md:text-xs tracking-wider">Tarefas</p>
                </Card>

                <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4 md:p-6">
                  <div className="flex items-center justify-between mb-2 md:mb-4">
                    <TrendingUp className="w-6 h-6 md:w-8 md:h-8 text-[#00c896]" />
                    <span className="font-data text-lg md:text-2xl">{stats.habits_completed_today ?? 0}/{stats.habits_total ?? 0}</span>
                  </div>
                  <p className="text-[#888888] uppercase text-[10px] md:text-xs tracking-wider">Hábitos</p>
                </Card>
                ...
                <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4 md:p-6">
                  ...
                  <p className="text-[#888888] uppercase text-[10px] md:text-xs tracking-wider">Saldo</p>
                </Card>

                <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4 md:p-6">
                  <div className="flex items-center justify-between mb-2 md:mb-4">
                    <Target className="w-6 h-6 md:w-8 md:h-8 text-[#00c896]" />
                    <span className="font-data text-lg md:text-2xl">{(stats.goals_avg_progress ?? 0).toFixed(0)}%</span>
                  </div>
                  <p className="text-[#888888] uppercase text-[10px] md:text-xs tracking-wider">Metas</p>
                </Card>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 mb-6 md:mb-8">
                ...
                <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4 md:p-6">
                  <div className="flex items-center space-x-3 md:space-x-4 mb-3 md:mb-4">
                    ...
                    <div>
                      <p className="text-sm text-[#888888] uppercase tracking-wider mb-1">Treinos</p>
                      <p className="font-heading text-2xl">Esta Semana</p>
                    </div>
                  </div>
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="text-[#888888] flex items-center gap-2"><Flame className="w-4 h-4" /> Sessões</span>
                      <span className="font-data text-[#FF6B6B]">{stats.workout_stats?.workouts_this_week || 0}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-[#888888] flex items-center gap-2"><Clock className="w-4 h-4" /> Duração</span>
                      <span className="font-data">{stats.workout_stats?.total_duration_minutes || 0} min</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-[#888888] flex items-center gap-2"><Flame className="w-4 h-4" /> Calorias</span>
                ...
                <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4 md:p-6">
                  <div className="flex items-center space-x-3 md:space-x-4 mb-3 md:mb-4">
                    ...
                    <div>
                      <p className="text-sm text-[#888888] uppercase tracking-wider mb-1">Nutrição</p>
                  ...
                  <div className="space-y-3">
                    <div>
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-[#888888] text-sm">Calorias</span>
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-[#888888] text-sm flex items-center gap-1"><Droplets className="w-3 h-3" /> Água</span>
                ...
                <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4 md:p-6">
                  <div className="flex items-center space-x-3 md:space-x-4 mb-3 md:mb-4">
                    ...
                    <div>
                      <p className="text-sm text-[#888888] uppercase tracking-wider mb-1">Estudos</p>
                      <p className="font-heading text-2xl">Progresso</p>
                    </div>
                  </div>
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="text-[#888888] flex items-center gap-2"><Clock className="w-4 h-4" /> Hoje</span>
                      <span className="font-data">{stats.study_stats?.study_time_today_minutes || 0} min</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-[#888888] flex items-center gap-2"><Flame className="w-4 h-4" /> Streak</span>
                      <span className="font-data text-[#FFD700]">{stats.study_stats?.current_streak || 0} dias</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-[#888888] flex items-center gap-2"><Brain className="w-4 h-4" /> Flashcards</span>
                      <span className={`font-data ${(stats.study_stats?.flashcards_due || 0) > 0 ? 'text-[#FF9500]' : 'text-[#00c896]'}`}>
                        {stats.study_stats?.flashcards_due || 0} pendentes
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-[#888888] flex items-center gap-2"><Hash className="w-4 h-4" /> Matérias</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-6 md:mb-8">
                ...
                <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4 md:p-6">
                  <div className="flex items-center space-x-3 mb-3">
                    ...
                    <div>
                      <p className="text-xs text-[#888888] uppercase tracking-wider">Simulados</p>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-data text-[#00c896]">{simStats.total_simulados || 0}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-[#888888] text-sm">Tentativas</span>
                      <span className="font-data">{simStats.total_attempts || 0}</span>
                    </div>
                    <div>
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-[#888888] text-sm">Média</span>
                        <span className={`font-data text-sm ${(simStats.average_score || 0) >= 70 ? 'text-[#00c896]' : (simStats.average_score || 0) >= 50 ? 'text-[#FFD700]' : 'text-[#FF3B30]'}`}>
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-[#888888] text-sm">Melhor</span>
                      <span className="font-data text-[#00c896]">{simStats.best_score || 0}%</span>
                    </div>
                  </div>
                </Card>
                ...
                <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4 md:p-6">
                  <div className="flex items-center space-x-3 mb-3">
                    ...
                    <div>
                      <p className="text-xs text-[#888888] uppercase tracking-wider">Questões</p>
                  ...
                  <div className="space-y-3">
                    <div className="text-center py-2">
                      ...
                      <p className="text-xs text-[#888888] mt-1">total respondidas</p>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-center">
                      <div className="bg-[#1a1a1a] rounded-lg p-2">
                        <p className="font-data text-lg text-[#00c896]">{qOverview.total_correct || 0}</p>
                        <p className="text-[10px] text-[#888888]">Acertos</p>
                      </div>
                      <div className="bg-[#1a1a1a] rounded-lg p-2">
                        <p className={`font-data text-lg ${(qOverview.accuracy_rate || 0) >= 70 ? 'text-[#00c896]' : (qOverview.accuracy_rate || 0) >= 50 ? 'text-[#FFD700]' : 'text-[#FF3B30]'}`}>
                          {qOverview.accuracy_rate || 0}%
                        </p>
                        <p className="text-[10px] text-[#888888]">Taxa</p>
                      </div>
                    </div>
                  </div>
                </Card>
                ...
                <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4 md:p-6">
                  <div className="flex items-center space-x-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-[#888888] uppercase tracking-wider">Rank</p>
                      <p className="font-heading text-lg">{user?.rank || 'Recruta'}</p>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <p className="font-data text-xl">{user?.xp ?? 0}</p>
                      <p className="text-[10px] text-[#888888]">XP</p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex justify-between text-xs">
                      <span className="text-[#888888]">Próximo: {nextRank.name}</span>
                      <span className="font-data text-[#888888]">{nextRank.xp} XP</span>
                    </div>
                ...
                <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4 md:p-6">
                  <div className="flex items-center space-x-3 mb-3">
                    ...
                    <div>
                      <p className="text-xs text-[#888888] uppercase tracking-wider">Finanças</p>
                      <p className="font-heading text-lg">Mês Atual</p>
                    </div>
                  </div>
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="text-[#888888] text-sm">Receitas</span>
                      <span className="font-data text-sm text-[#00c896]">+R$ {(stats.income ?? 0).toFixed(0)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-[#888888] text-sm">Despesas</span>
                      <span className="font-data text-sm text-[#FF3B30]">-R$ {(stats.expenses ?? 0).toFixed(0)}</span>
                    </div>
                    <div className="border-t border-[#2a2a2a] pt-2 flex justify-between items-center">
                      <span className="font-medium text-sm">Saldo</span>
                      <span className={`font-data text-base ${(stats.balance ?? 0) >= 0 ? 'text-[#00c896]' : 'text-[#FF3B30]'}`}>
              ...
              {/* Weekly Summary */}
              {weeklySummary && (
                <Card className="bg-gradient-to-r from-[#0d0d0d] to-[#0a0a1a] border-[#2a2a2a] p-6">
                  <div className="flex items-center gap-3 mb-4">
                    <BarChart3 className="w-6 h-6 text-[#00c896]" />
                    <h2 className="font-heading text-lg">RESUMO SEMANAL</h2>
                    <span className="text-xs text-[#555555]">{weeklySummary.period?.start} → {weeklySummary.period?.end}</span>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    <div className="bg-[#1a1a1a] rounded-lg p-3 text-center">
                      <DollarSign className="w-5 h-5 mx-auto mb-1 text-[#00c896]" />
                      <p className={`font-data text-lg ${(weeklySummary.finance?.balance || 0) >= 0 ? 'text-[#00c896]' : 'text-[#FF3B30]'}`}>
                        R$ {(weeklySummary.finance?.balance || 0).toFixed(0)}
                      </p>
                      <p className="text-[10px] text-[#555555]">Saldo Semana</p>
                    </div>
                    <div className="bg-[#1a1a1a] rounded-lg p-3 text-center">
                      ...
                      <p className="text-[10px] text-[#555555]">Treinos ({weeklySummary.workouts?.total_minutes || 0}min)</p>
                    </div>
                    <div className="bg-[#1a1a1a] rounded-lg p-3 text-center">
                      ...
                      <p className="text-[10px] text-[#555555]">Hábitos</p>
                    </div>
                    <div className="bg-[#1a1a1a] rounded-lg p-3 text-center">
                      <CheckSquare className="w-5 h-5 mx-auto mb-1 text-[#00c896]" />
                      <p className="font-data text-lg text-white">{weeklySummary.tasks?.completed || 0}/{weeklySummary.tasks?.total || 0}</p>
                      <p className="text-[10px] text-[#555555]">Tarefas</p>
                    </div>
                    <div className="bg-[#1a1a1a] rounded-lg p-3 text-center">
                      ...
                      <p className="text-[10px] text-[#555555]">Min Estudados</p>
                    </div>
                  </div>
                </Card>
              )}
          ...
          {analytics && analytics.data && analytics.data.length > 0 && (
            <div className="mt-8">
              <div className="flex items-center justify-between mb-5">
                ...
                <div className="flex gap-1 bg-[#0d0d0d] border border-[#2a2a2a] rounded-lg p-1">
                  {[7, 14, 30].map(d => (
                    <button
                      key={d}
                      onClick={() => fetchAnalytics(d)}
                      className={"px-3 py-1 text-xs rounded-md transition-colors " + (analyticsDays === d ? "bg-[#00c896] text-white" : "text-[#555555] hover:text-white")}
                    >
                      {d}d
                    </button>
                  ))}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4 md:p-5">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-sm font-medium text-[#888888] uppercase tracking-wider">Tarefas & Hábitos</h3>
                  </div>
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={analytics.data} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#1A1A1A" />
                      <XAxis dataKey="label" tick={{ fill: "#555555", fontSize: 11 }} axisLine={{ stroke: "#2a2a2a" }} />
                      <YAxis tick={{ fill: "#555555", fontSize: 11 }} axisLine={{ stroke: "#2a2a2a" }} />
                      <Tooltip
                        contentStyle={{ background: "#0d0d0d", border: "1px solid #2a2a2a", borderRadius: 8, color: "#fff", fontSize: 12 }}
                        labelStyle={{ color: "#888888" }}
                      />
                      <Bar dataKey="tasks" name="Tarefas" fill="#00c896" radius={[4, 4, 0, 0]} />
                      <Bar dataKey="habits" name="Hábitos" fill="#00c896" radius={[4, 4, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </Card>
                ...
                <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4 md:p-5">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-sm font-medium text-[#888888] uppercase tracking-wider">Receitas vs Despesas</h3>
                  </div>
                  <ResponsiveContainer width="100%" height={220}>
                    <AreaChart data={analytics.data} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
                      ...
                      />
                      <Area type="monotone" dataKey="income" name="Receitas" stroke="#00c896" fill="url(#gradIncome)" strokeWidth={2} />
                ...
                <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4 md:p-5">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-sm font-medium text-[#888888] uppercase tracking-wider">Tempo de Estudo (min)</h3>
                  </div>
                  <ResponsiveContainer width="100%" height={220}>
                    <AreaChart data={analytics.data} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
                      <defs>
                ...
                <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4 md:p-5">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-sm font-medium text-[#888888] uppercase tracking-wider">XP Ganho</h3>
                  </div>
                  <ResponsiveContainer width="100%" height={220}>
                    <LineChart data={analytics.data} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
                      ...
                      <Line type="monotone" dataKey="xp_cumulative" name="XP Acumulado" stroke="#00c896" strokeWidth={2} dot={false} strokeDasharray="5 5" />
                    </LineChart>
                  </ResponsiveContainer>
                </Card>
              </div>
              ...
              {analytics.totals && (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
                  <div className="bg-[#0d0d0d] border border-[#2a2a2a] rounded-lg p-3 text-center">
                    <p className="font-data text-xl text-[#00c896]">{analytics.totals.tasks}</p>
                    <p className="text-[10px] text-[#555555] uppercase">Tarefas Feitas</p>
                  </div>
                  <div className="bg-[#0d0d0d] border border-[#2a2a2a] rounded-lg p-3 text-center">
                    <p className="font-data text-xl text-[#A78BFA]">{analytics.totals.study_hours}h</p>
                    <p className="text-[10px] text-[#555555] uppercase">Horas Estudadas</p>
                  </div>
                  <div className="bg-[#0d0d0d] border border-[#2a2a2a] rounded-lg p-3 text-center">
                    <p className="font-data text-xl text-[#FF6B6B]">{analytics.totals.workouts}</p>
                    <p className="text-[10px] text-[#555555] uppercase">Treinos</p>
                  </div>
                  <div className="bg-[#0d0d0d] border border-[#2a2a2a] rounded-lg p-3 text-center">
                    <p className="font-data text-xl text-[#FFD700]">{analytics.totals.xp_earned}</p>
                    <p className="text-[10px] text-[#555555] uppercase">XP Ganho</p>
                  </div>
                </div>
              )}
            </div>
