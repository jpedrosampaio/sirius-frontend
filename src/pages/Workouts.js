...
export default function Workouts() {
  ...
  const WorkoutCard = ({ workout, showDate = false }) => {
    ...
    return (
      <Card key={workout.log_id} className={`bg-[#0d0d0d] border-[#2a2a2a] p-4 ${!workout.completed ? 'opacity-50' : ''}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-[#888888]">
                ...
                {hasExercises && (
                  <span className="ml-2 text-[#00c896]">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[#00c896] font-mono text-sm">{workout.completed ? '+' : ''}{workout.xp_earned} XP</span>
            {hasExercises && (
              <Button variant="ghost" size="sm" onClick={() => toggleWorkoutExpanded(workout.log_id)} className="text-[#888888]">
        ...
        {hasExercises && isExpanded && (
          <div className="mt-4 border-t border-[#2a2a2a] pt-4 space-y-2">
            <Label className="text-xs uppercase tracking-wider text-[#888888]">Exercícios do Treino</Label>
            {(workout.exercises_completed || []).map((ex, idx) => (
              <div key={idx} className={`flex items-center gap-3 p-2 rounded ${ex.completed ? 'bg-[#1a1a1a]' : 'bg-[#0d0d0d] border border-[#2a2a2a]'}`}>
                <div className={`w-5 h-5 rounded border flex items-center justify-center ${ex.completed ? 'bg-[#00c896] border-[#00c896]' : 'border-[#555555]'}`}>
                  {ex.completed && <Check className="w-3 h-3 text-black" />}
                </div>
                <span className={`font-mono text-sm flex-1 ${ex.completed ? 'text-white' : 'text-[#888888]'}`}>
    ...
    <div className="flex min-h-screen bg-[#050505]">
      ...
      <div className="flex-1 ml-0 md:ml-64 p-4 md:p-8 pb-24 md:pb-8 pt-[72px] md:pt-8">
        <div className="max-w-6xl mx-auto">
          {/* Motivational Quote */}
          {motivationalQuote && (
            <Card className="bg-gradient-to-r from-[#0d0d0d] to-[#1a1a2e] border-[#2a2a2a] p-4 mb-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Sparkles className="w-6 h-6 text-[#00c896]" />
                  <p className="text-lg italic text-white">{motivationalQuote.quote}</p>
                </div>
                <Button variant="ghost" size="sm" onClick={refreshQuote} className="text-[#888888]">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
              ...
              <p className="text-[#888888]">Registre e acompanhe sua evolução física</p>
            </div>
            <div className="flex gap-2 flex-wrap">
              <Dialog open={openLog} onOpenChange={setOpenLog}>
                <DialogTrigger asChild>
                  <Button data-testid="log-workout-btn" className="bg-[#00c896] hover:bg-[#00D4E5] text-black">
                ...
                <DialogContent className="bg-[#0d0d0d] border-[#2a2a2a] text-white max-w-lg max-h-[90vh] overflow-y-auto">
                  ...
                  <div className="space-y-4 mt-4">
                    ...
                    <div>
                      ...
                      <Input value={newWorkout.name} onChange={(e) => setNewWorkout({...newWorkout, name: e.target.value})} placeholder="Ex: Treino de Peito" className="bg-[#1a1a1a] border-[#2a2a2a] text-white mt-1" />
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <Label className="text-xs uppercase tracking-wider">Duração (min)</Label>
                        <Input type="number" value={newWorkout.duration_minutes} onChange={(e) => setNewWorkout({...newWorkout, duration_minutes: parseInt(e.target.value) || 0})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white mt-1" />
                      </div>
                      <div>
                        <Label className="text-xs uppercase tracking-wider">Data</Label>
                        <Input type="date" value={newWorkout.date} onChange={(e) => setNewWorkout({...newWorkout, date: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white mt-1" />
                      </div>
                    </div>
                        ...
                        <div className="space-y-2 max-h-48 overflow-y-auto">
                          {newWorkout.exercises_completed.map((ex, idx) => (
                            <div 
                              ...
                              className={`flex items-center gap-3 p-3 rounded cursor-pointer transition-colors ${ex.completed ? 'bg-[#1a2f1a] border border-green-900' : 'bg-[#1a1a1a] border border-[#2a2a2a] hover:border-[#333333]'}`}
                            >
                              <Checkbox 
                                checked={ex.completed} 
                                onCheckedChange={() => toggleExerciseInNewWorkout(idx)}
                                className="border-[#555555] data-[state=checked]:bg-[#00c896] data-[state=checked]:border-[#00c896]"
                              />
                    ...
                    <div>
                      ...
                      <Textarea value={newWorkout.notes} onChange={(e) => setNewWorkout({...newWorkout, notes: e.target.value})} placeholder="Como foi o treino?" className="bg-[#1a1a1a] border-[#2a2a2a] text-white mt-1" />
                    </div>
                    <Button onClick={handleLogWorkout} className="w-full bg-[#00c896] hover:bg-[#00D4E5] text-black">Registrar Treino</Button>
                  </div>
                </DialogContent>
              </Dialog>
              ...
              <Dialog open={openPlan} onOpenChange={setOpenPlan}>
                <DialogTrigger asChild>
                  <Button data-testid="create-plan-btn" variant="outline" className="border-[#2a2a2a]">
                ...
                <DialogContent className="bg-[#0d0d0d] border-[#2a2a2a] text-white max-w-lg max-h-[90vh] overflow-y-auto">
                  ...
                  <div className="space-y-4 mt-4">
                    <div>
                      ...
                      <Input value={newPlan.name} onChange={(e) => setNewPlan({...newPlan, name: e.target.value})} placeholder="Ex: Treino A - Peito e Tríceps" className="bg-[#1a1a1a] border-[#2a2a2a] text-white mt-1" />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <Textarea value={newPlan.description} onChange={(e) => setNewPlan({...newPlan, description: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white mt-1" />
                    </div>
                    <div className="border-t border-[#2a2a2a] pt-4">
                      <Label className="text-xs uppercase tracking-wider mb-2 block">Adicionar Exercício</Label>
                      <div className="space-y-2">
                        <Input value={newExercise.name} onChange={(e) => setNewExercise({...newExercise, name: e.target.value})} placeholder="Nome do exercício" className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                        <div className="grid grid-cols-3 gap-2">
                          <div>
                            <Label className="text-xs text-[#888888]">Séries</Label>
                            <Input type="number" value={newExercise.sets} onChange={(e) => setNewExercise({...newExercise, sets: parseInt(e.target.value) || 0})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                          </div>
                          <div>
                            <Label className="text-xs text-[#888888]">Reps</Label>
                            <Input type="number" value={newExercise.reps} onChange={(e) => setNewExercise({...newExercise, reps: parseInt(e.target.value) || 0})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                          </div>
                          <div>
                            <Label className="text-xs text-[#888888]">Carga</Label>
                            <Input value={newExercise.weight} onChange={(e) => setNewExercise({...newExercise, weight: e.target.value})} placeholder="Ex: 20kg" className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                          </div>
                        </div>
                        <Button onClick={addExerciseToPlan} variant="outline" className="w-full border-[#2a2a2a]">
                    ...
                    {newPlan.exercises.length > 0 && (
                      <div className="space-y-2">
                        ...
                        {newPlan.exercises.map((ex, idx) => (
                          <div key={idx} className="bg-[#1a1a1a] p-3 rounded flex items-center justify-between">
                    ...
                    <Button onClick={handleCreatePlan} className="w-full bg-[#00c896] hover:bg-[#00D4E5] text-black">Criar Ficha</Button>
                  </div>
                </DialogContent>
              </Dialog>
              ...
              <Dialog open={openAiGenerate} onOpenChange={setOpenAiGenerate}>
                <DialogTrigger asChild>
                  <Button data-testid="open-ai-generate-btn" className="bg-gradient-to-r from-[#A855F7] to-[#00c896] hover:opacity-90 text-white">
                    <Sparkles className="w-4 h-4 mr-2" /> Gerar com IA
                  </Button>
                </DialogTrigger>
                <DialogContent className="bg-[#0d0d0d] border-[#2a2a2a] text-white max-w-2xl max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
                  ...
                  <div className="space-y-4 mt-4">
                    {/* Mode Selector Tabs */}
                    <div className="flex gap-2 p-1 bg-[#1a1a1a] rounded-lg border border-[#2a2a2a]">
                      <button
                      ...
                      <button
                        ...
                        className={`flex-1 py-2.5 px-4 rounded-md text-sm font-medium transition-all flex items-center justify-center gap-2 ${
                          aiGenMode === "periodo" 
                            ? 'bg-gradient-to-r from-[#00c896] to-[#0EA5E9] text-white shadow-lg' 
                            : 'text-[#888888] hover:text-white hover:bg-[#1A1A1A]'
                        }`}
                      >
                    ...
                    <div>
                      <Label className="text-xs uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                        ...
                        <span className="text-[#555555] font-normal normal-case">(opcional)</span>
                      </Label>
                      <textarea
                        ...
                        className="w-full bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg p-3 text-sm text-white placeholder:text-[#555555] focus:border-[#A855F7] focus:ring-1 focus:ring-[#A855F7] outline-none resize-none transition-colors"
                        rows={2}
                      />
                    ...
                    {aiGenMode === "tipo_treino" && (
                      <div className="space-y-4">
                              ...
                              <button
                                ...
                                className={`flex-1 py-2.5 px-3 rounded-lg text-sm font-bold transition-all border ${
                                  aiGenForm.split_type === opt.value
                                    ...
                                    : 'bg-[#1a1a1a] border-[#2a2a2a] text-[#888888] hover:border-[#A855F7]/50'
                                }`}
                              >
                                {opt.value}
                              </button>
                        ...
                        <div>
                          ...
                          <div className="space-y-3">
                            {aiGenForm.split_config.map((split, splitIdx) => (
                              <div key={split.label} className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg p-3">
                                <div className="flex items-center gap-2 mb-2">
                                  <span className="w-8 h-8 flex items-center justify-center rounded-lg bg-[#A855F7]/20 text-[#A855F7] font-bold text-sm">
                                    {split.label}
                                  </span>
                                  <span className="text-sm text-[#888888] flex-1 truncate">
                                    ...
                                    <button
                                      ...
                                      className={`px-2.5 py-1 rounded-full text-[11px] font-medium transition-all ${
                                        split.muscle_groups.includes(mg.value)
                                          ...
                                          : 'bg-[#0d0d0d] border border-[#2a2a2a] text-[#888888] hover:border-[#A855F7]/50 hover:text-[#888888]'
                                      }`}
                                    >
                                      {mg.emoji} {mg.label}
                                    </button>
                                    ...
                                    <button
                                      ...
                                      className={`text-left p-2.5 rounded-lg border transition-all ${
                                        aiGenForm.cardio_mode === mode.value
                                          ...
                                          : 'bg-[#0d0d0d] border-[#2a2a2a] text-[#888888] hover:border-[#00c896]/50'
                                      }`}
                                    >
                                      <span className="text-sm font-medium">{mode.icon} {mode.label}</span>
                                      <p className="text-[11px] text-[#888888] mt-0.5">{mode.desc}</p>
                                    </button>
                                  ))}
                                </div>
                              </div>
                                    ...
                                    <button
                                      ...
                                      className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                                        aiGenForm.cardio_type === ct.value
                                          ...
                                          : 'bg-[#0d0d0d] border border-[#2a2a2a] text-[#888888] hover:border-[#00c896]/50'
                                      }`}
                                    >
                                      {ct.emoji} {ct.label}
                                    </button>
                        ...
                        {/* Summary box */}
                        <div className="bg-[#1a1a1a] border border-[#A855F7]/30 rounded-lg p-4">
                          <div className="flex items-center gap-2 mb-2">
                          ...
                          <ul className="text-xs text-[#888888] space-y-1">
                            ...
                            {aiGenForm.include_cardio && <li>• Cardio: <span className="text-[#00c896] font-medium">{
                    ...
                    {aiGenMode === "periodo" && (
                      <div className="space-y-4">
                        ...
                        <div>
                              ...
                              <button
                                ...
                                className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                                  aiGenForm.muscle_groups.includes(mg.value)
                                    ? 'bg-[#A855F7] text-white'
                                    : 'bg-[#1a1a1a] border border-[#2a2a2a] text-[#888888] hover:border-[#A855F7]'
                                }`}
                              >
                                {mg.emoji} {mg.label}
                              </button>
                          ...
                          <p className="text-xs text-[#555555] mt-2">Deixe vazio para um treino completo</p>
                        </div>
                    ...
                    <Button 
                      ...
                      className="w-full bg-gradient-to-r from-[#A855F7] to-[#00c896] hover:opacity-90 text-white h-11"
                    >
                      {generatingPlan ? (
                ...
                <div className="space-y-4 mt-4">
                    ...
                    <div className="border-t border-[#2a2a2a] pt-4">
                      ...
                      <div className="space-y-3">
                        {editingPlan.exercises.map((ex, idx) => (
                          <div key={idx} className="bg-[#1a1a1a] p-3 rounded border border-[#2a2a2a]">
                            <div className="flex items-center justify-between mb-2">
                              <Input 
                                value={ex.name} 
                                onChange={(e) => updateExerciseInEditPlan(idx, 'name', e.target.value)}
                                className="bg-[#0d0d0d] border-[#2a2a2a] text-white flex-1 mr-2"
                  ...
                  <div className="border-t border-[#2a2a2a] pt-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Button onClick={addExerciseToEditPlan} variant="outline" className="w-full border-[#2a2a2a]">
                        <Plus className="w-4 h-4 mr-2" /> Adicionar Exercício
                      </Button>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <Button onClick={() => setOpenEditPlan(false)} variant="outline" className="flex-1 border-[#2a2a2a]">
                      Cancelar
                    </Button>
                    <Button onClick={handleUpdatePlan} className="flex-1 bg-[#00c896] hover:bg-[#00D4E5] text-black">
                      Salvar Alterações
                    </Button>
                  </div>
                </div>
          ...
          <Dialog open={openCompleteWorkout} onOpenChange={setOpenCompleteWorkout}>
            <DialogContent className="bg-[#0d0d0d] border-[#2a2a2a] text-white max-w-md">
              ...
              {completingPlan && (
                <div className="space-y-4 mt-4">
                  <p className="text-[#888888]">Confirme os dados do treino <span className="text-white font-semibold">{completingPlan.name}</span></p>
                  ...
                  <div className="flex gap-2">
                    <Button onClick={() => setOpenCompleteWorkout(false)} variant="outline" className="flex-1 border-[#2a2a2a]">
                      Cancelar
                    </Button>
                    <Button onClick={handleCompleteWorkout} className="flex-1 bg-[#00c896] hover:bg-[#00D4E5] text-black">
          ...
          {stats && (
            <div className="flex items-center gap-6 p-4 bg-[#0d0d0d] border border-[#2a2a2a] rounded-lg mb-6">
              <div className="flex items-center gap-2">
                <Dumbbell className="w-5 h-5 text-[#00c896]" />
                <span className="text-xs text-[#888888] uppercase">Treinos</span>
                <span className="font-heading text-xl">{stats.total_workouts}</span>
              </div>
              <div className="h-6 w-px bg-[#2a2a2a]" />
              <div className="flex items-center gap-2">
                <Timer className="w-5 h-5 text-[#F59E0B]" />
                <span className="text-xs text-[#888888] uppercase">Min</span>
                <span className="font-heading text-xl">{stats.total_duration_minutes}</span>
              </div>
              <div className="h-6 w-px bg-[#2a2a2a]" />
              <div className="flex items-center gap-2">
                <Flame className="w-5 h-5 text-[#EF4444]" />
                <span className="text-xs text-[#888888] uppercase">Cal</span>
                <span className="font-heading text-xl">{stats.total_calories}</span>
              </div>
              <div className="h-6 w-px bg-[#2a2a2a]" />
              <div className="flex items-center gap-2">
                <TrendingUp className="w-5 h-5 text-[#22C55E]" />
                <span className="text-xs text-[#888888] uppercase">XP</span>
              ...
              {detailedStats && (
                <>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-[#888888] uppercase">Streak</span>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="bg-[#0d0d0d] border border-[#2a2a2a] mb-6 overflow-x-auto flex-nowrap w-full justify-start md:justify-center">
              <TabsTrigger value="log" className="data-[state=active]:bg-[#2a2a2a]">
                <Activity className="w-4 h-4 mr-2" /> Hoje
              </TabsTrigger>
              <TabsTrigger value="plans" className="data-[state=active]:bg-[#2a2a2a]">
                <FileText className="w-4 h-4 mr-2" /> Fichas
              </TabsTrigger>
              <TabsTrigger value="stats" className="data-[state=active]:bg-[#2a2a2a]">
                <BarChart3 className="w-4 h-4 mr-2" /> Estatísticas
              </TabsTrigger>
              <TabsTrigger value="evolution" className="data-[state=active]:bg-[#2a2a2a]">
                <Scale className="w-4 h-4 mr-2" /> Evolução
              </TabsTrigger>
              <TabsTrigger value="history" className="data-[state=active]:bg-[#2a2a2a]">
                <Calendar className="w-4 h-4 mr-2" /> Histórico
              </TabsTrigger>
              <TabsTrigger value="saved_insights" className="data-[state=active]:bg-[#2a2a2a]" onClick={fetchSavedInsights}>
                <BookOpen className="w-4 h-4 mr-2" /> Insights
              </TabsTrigger>
              {activeSession && (
                <TabsTrigger value="session" className="data-[state=active]:bg-[#2a2a2a] text-green-400 animate-pulse">
              ...
              <div className="grid gap-4">
                {workouts.filter(w => w.date === today).length === 0 ? (
                  <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-8 text-center">
                    <Dumbbell className="w-12 h-12 text-[#555555] mx-auto mb-4" />
                    <p className="text-[#888888]">Nenhum treino registrado hoje</p>
                    <p className="text-sm text-[#555555] mt-2">Selecione uma ficha na aba "Fichas" ou clique em "Registrar Treino"</p>
                  </Card>
                ) : (
            ...
            <TabsContent value="plans">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {plans.length === 0 ? (
                  <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-8 text-center md:col-span-2">
                    <FileText className="w-12 h-12 text-[#555555] mx-auto mb-4" />
                    <p className="text-[#888888]">Nenhuma ficha de treino criada</p>
                    <p className="text-sm text-[#555555] mt-2">Clique em "Nova Ficha" para criar uma</p>
                  </Card>
                ) : (
                  plans.map(plan => {
                    ...
                    return (
                      <Card key={plan.plan_id} className={`bg-[#0d0d0d] border-[#2a2a2a] p-4 ${isCompleted ? 'border-green-900 bg-[#0a1a0a]' : ''} ${plan.generated_by_ai ? 'border-l-2 border-l-[#A855F7]' : ''}`}>
                        <div 
                        ...
                        >
                          <div className="flex justify-between items-start">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                {plan.plan_duration && plan.plan_duration !== "dia" && (
                                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#00c896]/10 text-[#00c896] border border-[#00c896]/30">
                              ...
                              {plan.description && <p className="text-sm text-[#888888] mt-1">{plan.description}</p>}
                              {plan.generation_mode === "tipo_treino" && plan.split_config && (
                                <div className="flex flex-wrap gap-1 mt-1.5">
                                  {plan.split_config.map((s) => (
                                    <span key={s.label} className="text-[10px] px-1.5 py-0.5 rounded bg-[#1A1A2E] text-[#888888] border border-[#2a2a2a]">
                                      <span className="text-[#A855F7] font-bold">{s.label}</span> {s.name}
                                    </span>
                                  ))}
                                  {plan.training_days_per_week && <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#1A1A2E] text-[#888888]">{plan.training_days_per_week}x/sem</span>}
                                  {plan.cycle_weeks && <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#1A1A2E] text-[#888888]">{plan.cycle_weeks} sem</span>}
                                </div>
                              )}
                              <p className="text-xs text-[#555555] mt-1">
                                {(plan.days && plan.days.length > 0) ? `${plan.days.length} dias · ` : ''}{(plan.exercises || []).length} exercícios
                                {total > 0 && (
                                  <span className={`ml-2 ${isCompleted ? 'text-green-500' : 'text-[#00c896]'}`}>
                            ...
                            <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
                              ...
                              <Button variant="ghost" size="sm" onClick={() => openEditDialog(plan)} className="text-[#00c896] h-8 w-8 p-0">
                          ...
                          <div className="mt-4 border-t border-[#2a2a2a] pt-4">
                            ...
                            {plan.days && plan.days.length > 1 && (() => {
                              ...
                              return (
                                <div className="mb-4 space-y-3">
                                  ...
                                  {hasMultipleWeeks && (
                                    <div>
                                      <Label className="text-xs uppercase tracking-wider text-[#888888] mb-2 block">Semana</Label>
                                      ...
                                      {currentWeekProgression && (
                                        <div className="mt-2 px-3 py-2 bg-[#1A1A2E] rounded-lg border border-[#2a2a2a]">
                                          <p className="text-xs text-[#A855F7] font-medium">{currentWeekProgression.focus}</p>
                                          <p className="text-[11px] text-[#888888] mt-0.5">{currentWeekProgression.notes}</p>
                                        </div>
                                      )}
                                    </div>
                                  )}
                                  ...
                                  <div>
                                    <Label className="text-xs uppercase tracking-wider text-[#888888] mb-2 block">
                                          ...
                                          <button
                                            ...
                                            className={`px-3 py-2 text-xs rounded-lg transition-all ${
                                              selectedDayIndex === globalIdx 
                                                ? isCardio ? 'bg-green-600 text-white font-medium' : 'bg-[#00c896] text-black font-medium' 
                                                : 'bg-[#1a1a1a] border border-[#2a2a2a] text-[#888888] hover:border-[#00c896]'
                                            }`}
                                          >
                            ...
                            <div className="flex justify-between items-center mb-3">
                              ...
                              <Button 
                                ...
                                className="text-xs text-[#888888] h-6 px-2"
                              >
                                Resetar
                              </Button>
                            </div>
                                  ...
                                  return (
                                    <div key={idx} className="space-y-0">
                                      ...
                                      >
                                        <div onClick={() => !isCompleted && toggleDailyExercise(plan.plan_id, idx)} className="flex items-center gap-3 flex-1">
                                          <Checkbox 
                                            ...
                                            className="border-[#555555] data-[state=checked]:bg-[#00c896] data-[state=checked]:border-[#00c896]"
                                          />
                                          <span className="text-[#555555] font-mono text-sm">{idx + 1}.</span>
                                          <div className="flex-1">
                                            <span className={`font-mono text-sm ${isChecked ? 'text-green-400 line-through' : 'text-white'}`}>
                                              {ex.name} - {ex.sets}x{ex.reps} {ex.weight && `@ ${ex.weight}`}
                                            </span>
                                            {ex.muscle_group && (
                                              <span className="text-[10px] ml-2 px-1.5 py-0.5 rounded bg-[#2a2a2a] text-[#888888]">{ex.muscle_group}</span>
                                            )}
                                            {ex.rest_seconds && (
                                              <span className="text-[10px] ml-1 text-[#555555]">⏱ {ex.rest_seconds}s</span>
                                            )}
                                          </div>
                                        </div>
                                        {hasTutorial && (
                                          <Button 
                                            ...
                                            className={`h-7 w-7 p-0 ${expandedTutorials[tutorialKey] ? 'text-[#A855F7]' : 'text-[#555555]'}`}
                                            title="Ver tutorial"
                                          >
                                      ...
                                      {hasTutorial && expandedTutorials[tutorialKey] && (
                                        <div className="bg-[#0a0a1a] border border-[#2a2a2a] border-t-0 rounded-b p-3 space-y-2">
                                          {ex.tutorial && (
                                            <div>
                                              ...
                                              <p className="text-xs text-[#888888] leading-relaxed">{ex.tutorial}</p>
                                            </div>
                                          )}
                                        </div>
                                      )}
                            ...
                            {total > 0 && (
                              <div className="mt-4">
                                ...
                                <div className="h-2 bg-[#1a1a1a] rounded-full overflow-hidden">
                                  <div 
                                    className="h-full bg-gradient-to-r from-[#00c896] to-[#22C55E] transition-all duration-300"
                  ...
                  <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-6">
                    <div className="flex items-center justify-between mb-6">
                      <div>
                        ...
                        <p className="text-sm text-[#888888]">Últimos 30 dias de treino</p>
                      </div>
                      <div className="text-right">
                        <span className="font-data text-4xl text-[#00c896]">{detailedStats.consistency_percentage}%</span>
                        <p className="text-xs text-[#888888]">{detailedStats.trained_days} de 30 dias</p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-7 gap-2">
                      {['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'].map(day => (
                        <div key={day} className="text-center text-xs text-[#555555] pb-2">{day}</div>
                      ))}
                        ...
                        return (
                          <div
                            ...
                            className={`aspect-square rounded-lg flex items-center justify-center transition-all hover:scale-105 cursor-pointer ${
                              day.count > 0 ? 'bg-[#00c896]' : 'bg-[#1a1a1a] border border-[#2a2a2a]'
                            }`}
                            style={{
                ...
                {detailedStats && (
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4 text-center">
                      ...
                      <div className="text-xs text-[#888888] uppercase">Streak Atual</div>
                    </Card>
                    <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4 text-center">
                      ...
                      <div className="text-xs text-[#888888] uppercase">Melhor Streak</div>
                    </Card>
                    <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4 text-center">
                      ...
                      <div className="text-xs text-[#888888] uppercase">Min/Treino</div>
                    </Card>
                    <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4 text-center">
                      ...
                      <div className="text-xs text-[#888888] uppercase">Cal/Treino</div>
                    </Card>
                  </div>
                )}
                ...
                {detailedStats && detailedStats.daily_data && (
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4">
                      <h3 className="font-heading text-sm mb-3 flex items-center gap-2"><Activity className="w-4 h-4 text-[#00c896]" />Frequência de Treino (30 dias)</h3>
                      <ResponsiveContainer width="100%" height={200}>
                        <BarChart data={detailedStats.daily_data.filter((_, i) => i % 2 === 0)}>
                          <CartesianGrid strokeDasharray="3 3" stroke="#2a2a2a" />
                          <XAxis dataKey="date" tick={{ fill: '#888888', fontSize: 9 }} tickFormatter={(v) => v.slice(5)} />
                          ...
                          <Bar dataKey="count" fill="#00c896" name="Treinos" radius={[2, 2, 0, 0]} />
                        </BarChart>
                      </ResponsiveContainer>
                    </Card>
                    ...
                    <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4">
                      ...
                      <div className="grid grid-cols-3 gap-3 text-center">
                        <div>
                          ...
                          <p className="text-[10px] text-[#888888] uppercase">Sequência Atual</p>
                        </div>
                        <div>
                          <p className="font-data text-2xl text-[#00c896]">{detailedStats.best_streak}</p>
                          <p className="text-[10px] text-[#888888] uppercase">Melhor Sequência</p>
                        </div>
                        <div>
                          <p className="font-data text-2xl text-[#00c896]">{detailedStats.consistency_percentage}%</p>
                          <p className="text-[10px] text-[#888888] uppercase">Consistência</p>
                        </div>
                      </div>
                      <div className="mt-3">
                        ...
                        <div className="w-full bg-[#2a2a2a] rounded-full h-2">
                          <div className="h-2 rounded-full bg-gradient-to-r from-orange-500 to-[#00c896]" style={{ width: `${detailedStats.consistency_percentage}%` }}></div>
                        </div>
                      </div>
                    </Card>
                    ...
                    {stats && stats.by_activity_type && Object.keys(stats.by_activity_type).length > 0 && (
                      <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4">
                        <h3 className="font-heading text-sm mb-3 flex items-center gap-2"><Target className="w-4 h-4 text-[#00c896]" />Tipo de Atividade</h3>
                        <ResponsiveContainer width="100%" height={200}>
                          <PieChart>
                            <Pie
                            ...
                            >
                              {Object.keys(stats.by_activity_type).map((_, i) => (
                                <Cell key={i} fill={['#00c896', '#A855F7', '#00c896', '#FF9500', '#FF3B30', '#FFD700'][i % 6]} />
                              ))}
                            </Pie>
                            <Tooltip />
                          </PieChart>
                ...
                <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      ...
                      <p className="text-sm text-[#888888]">Recomendações personalizadas por IA</p>
                    </div>
                    <Button
                      onClick={getAiSuggestions}
                      disabled={loadingSuggestions}
                      className="bg-gradient-to-r from-[#A855F7] to-[#00c896] hover:opacity-90 text-white"
                    >
                      {loadingSuggestions ? (
                  ...
                  {aiSuggestions ? (
                    <div className="mt-4">
                      <div className="bg-[#1a1a1a] rounded-lg p-4 border border-[#2a2a2a]">
                        <p className="text-[#888888] whitespace-pre-wrap text-sm leading-relaxed">
                      ...
                      <div className="mt-3 flex justify-between items-center">
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button variant="ghost" size="sm" onClick={() => setAiSuggestions(null)} className="text-xs text-[#555555] hover:text-white">Limpar</Button>
                        </div>
                      </div>
                    </div>
                  ) : (
              ...
              <div className="grid gap-6">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div>
                    ...
                    <p className="text-sm text-[#888888]">Acompanhe suas medidas e progresso</p>
                  </div>
                  <div className="flex gap-2">
                    <Dialog open={openMeasurement} onOpenChange={setOpenMeasurement}>
                      <DialogTrigger asChild>
                        <Button className="bg-[#00c896] hover:bg-[#00D4E5] text-black">
                        ...
                        <div className="space-y-6 mt-4">
                          ...
                          <div className="border border-dashed border-[#2a2a2a] rounded-lg p-4 text-center">
                            ...
                            <label htmlFor="pdf-upload" className="cursor-pointer">
                              <Upload className="w-8 h-8 text-[#00c896] mx-auto mb-2" />
                              <p className="text-sm text-[#888888]">
                                {uploadingPdf ? "Analisando PDF..." : "Clique para importar PDF de avaliação física"}
                              </p>
                            </label>
                            {pdfAnalysis?.extracted_data?.recommendations && (
                              <div className="mt-4 text-left bg-[#1a1a1a] p-3 rounded">
                                <p className="text-xs text-[#00c896] uppercase mb-2">Recomendações do PDF:</p>
                                <p className="text-sm text-[#888888]">{(pdfAnalysis.extracted_data.recommendations || []).join(", ")}</p>
                              </div>
                            )}
                          </div>
                          ...
                          <div>
                            <Label className="text-xs uppercase tracking-wider">Data</Label>
                            <Input type="date" value={newMeasurement.date} onChange={(e) => setNewMeasurement({...newMeasurement, date: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white mt-1" />
                          </div>
                          
                          {/* Peso e Composição */}
                          <div>
                            <Label className="text-xs uppercase tracking-wider mb-3 block text-[#00c896]">Peso e Composição Corporal</Label>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                              <div>
                                <Label className="text-xs text-[#888888]">Peso (kg)</Label>
                                <Input type="number" step="0.1" value={newMeasurement.weight_kg} onChange={(e) => setNewMeasurement({...newMeasurement, weight_kg: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                              <div>
                                <Label className="text-xs text-[#888888]">Altura (cm)</Label>
                                <Input type="number" step="0.1" value={newMeasurement.height_cm} onChange={(e) => setNewMeasurement({...newMeasurement, height_cm: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                              <div>
                                <Label className="text-xs text-[#888888]">Gordura (%)</Label>
                                <Input type="number" step="0.1" value={newMeasurement.body_fat_percentage} onChange={(e) => setNewMeasurement({...newMeasurement, body_fat_percentage: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                              <div>
                                <Label className="text-xs text-[#888888]">Massa Musc. (kg)</Label>
                                <Input type="number" step="0.1" value={newMeasurement.muscle_mass_kg} onChange={(e) => setNewMeasurement({...newMeasurement, muscle_mass_kg: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                              <div>
                                <Label className="text-xs text-[#888888]">Massa Óssea (kg)</Label>
                                <Input type="number" step="0.1" value={newMeasurement.bone_mass_kg} onChange={(e) => setNewMeasurement({...newMeasurement, bone_mass_kg: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                              <div>
                                <Label className="text-xs text-[#888888]">Água (%)</Label>
                                <Input type="number" step="0.1" value={newMeasurement.water_percentage} onChange={(e) => setNewMeasurement({...newMeasurement, water_percentage: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                              <div>
                                <Label className="text-xs text-[#888888]">Gord. Visceral</Label>
                                <Input type="number" value={newMeasurement.visceral_fat} onChange={(e) => setNewMeasurement({...newMeasurement, visceral_fat: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                              <div>
                                <Label className="text-xs text-[#888888]">Idade Metab.</Label>
                                <Input type="number" value={newMeasurement.metabolic_age} onChange={(e) => setNewMeasurement({...newMeasurement, metabolic_age: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                            </div>
                          </div>
                          ...
                          {/* Medidas Corporais */}
                          <div>
                            <Label className="text-xs uppercase tracking-wider mb-3 block text-[#00c896]">Medidas Corporais (cm)</Label>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                              <div>
                                <Label className="text-xs text-[#888888]">Pescoço</Label>
                                <Input type="number" step="0.1" value={newMeasurement.neck_cm} onChange={(e) => setNewMeasurement({...newMeasurement, neck_cm: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                              <div>
                                <Label className="text-xs text-[#888888]">Ombros</Label>
                                <Input type="number" step="0.1" value={newMeasurement.shoulders_cm} onChange={(e) => setNewMeasurement({...newMeasurement, shoulders_cm: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                              <div>
                                <Label className="text-xs text-[#888888]">Peito</Label>
                                <Input type="number" step="0.1" value={newMeasurement.chest_cm} onChange={(e) => setNewMeasurement({...newMeasurement, chest_cm: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                              <div>
                                <Label className="text-xs text-[#888888]">Cintura</Label>
                                <Input type="number" step="0.1" value={newMeasurement.waist_cm} onChange={(e) => setNewMeasurement({...newMeasurement, waist_cm: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                              <div>
                                <Label className="text-xs text-[#888888]">Abdômen</Label>
                                <Input type="number" step="0.1" value={newMeasurement.abdomen_cm} onChange={(e) => setNewMeasurement({...newMeasurement, abdomen_cm: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                              <div>
                                <Label className="text-xs text-[#888888]">Quadril</Label>
                                <Input type="number" step="0.1" value={newMeasurement.hips_cm} onChange={(e) => setNewMeasurement({...newMeasurement, hips_cm: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                              <div>
                                <Label className="text-xs text-[#888888]">Braço Esq.</Label>
                                <Input type="number" step="0.1" value={newMeasurement.left_arm_cm} onChange={(e) => setNewMeasurement({...newMeasurement, left_arm_cm: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                              <div>
                                <Label className="text-xs text-[#888888]">Braço Dir.</Label>
                                <Input type="number" step="0.1" value={newMeasurement.right_arm_cm} onChange={(e) => setNewMeasurement({...newMeasurement, right_arm_cm: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                              <div>
                                <Label className="text-xs text-[#888888]">Antebraço Esq.</Label>
                                <Input type="number" step="0.1" value={newMeasurement.left_forearm_cm} onChange={(e) => setNewMeasurement({...newMeasurement, left_forearm_cm: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                              <div>
                                <Label className="text-xs text-[#888888]">Antebraço Dir.</Label>
                                <Input type="number" step="0.1" value={newMeasurement.right_forearm_cm} onChange={(e) => setNewMeasurement({...newMeasurement, right_forearm_cm: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                              <div>
                                <Label className="text-xs text-[#888888]">Coxa Esq.</Label>
                                <Input type="number" step="0.1" value={newMeasurement.left_thigh_cm} onChange={(e) => setNewMeasurement({...newMeasurement, left_thigh_cm: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                              <div>
                                <Label className="text-xs text-[#888888]">Coxa Dir.</Label>
                                <Input type="number" step="0.1" value={newMeasurement.right_thigh_cm} onChange={(e) => setNewMeasurement({...newMeasurement, right_thigh_cm: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                              <div>
                                <Label className="text-xs text-[#888888]">Panturrilha Esq.</Label>
                                <Input type="number" step="0.1" value={newMeasurement.left_calf_cm} onChange={(e) => setNewMeasurement({...newMeasurement, left_calf_cm: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                              <div>
                                <Label className="text-xs text-[#888888]">Panturrilha Dir.</Label>
                                <Input type="number" step="0.1" value={newMeasurement.right_calf_cm} onChange={(e) => setNewMeasurement({...newMeasurement, right_calf_cm: e.target.value})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white" />
                              </div>
                            </div>
                          </div>
                          ...
                          <div>
                            <Label className="text-xs uppercase tracking-wider">Observações</Label>
                            <Textarea value={newMeasurement.notes} onChange={(e) => setNewMeasurement({...newMeasurement, notes: e.target.value})} placeholder="Notas adicionais..." className="bg-[#1a1a1a] border-[#2a2a2a] text-white mt-1" />
                          </div>
                          
                          <Button onClick={handleCreateMeasurement} className="w-full bg-[#00c896] hover:bg-[#00D4E5] text-black">
                            Salvar Medidas
                          </Button>
                        </div>
                      </DialogContent>
                ...
                {latestMeasurement && (
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4">
                      <div className="flex items-center gap-3">
                        <Scale className="w-8 h-8 text-[#00c896]" />
                        <div>
                          <p className="text-xs text-[#888888] uppercase">Peso</p>
                    ...
                    <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs text-[#888888] uppercase">Gordura</p>
                    ...
                    <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs text-[#888888] uppercase">Cintura</p>
                ...
                <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-6">
                  ...
                  {recommendations ? (
                    <div className="prose prose-invert max-w-none">
                      <p className="text-[#888888] whitespace-pre-wrap">{recommendations.recommendations}</p>
                    </div>
                  ) : (
                    <p className="text-[#555555]">Clique em "Atualizar" para receber recomendações personalizadas baseadas em suas medidas e treinos.</p>
                  )}
                </Card>
                ...
                <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-6">
                  ...
                  {measurements.length === 0 ? (
                    <p className="text-[#555555] text-center py-4">Nenhuma medida registrada ainda</p>
                  ) : (
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-[#2a2a2a]">
                            <th className="text-left p-2 text-[#888888]">Data</th>
                            <th className="text-right p-2 text-[#888888]">Peso</th>
                            <th className="text-right p-2 text-[#888888]">IMC</th>
                            <th className="text-right p-2 text-[#888888]">Gordura</th>
                            <th className="text-right p-2 text-[#888888]">Cintura</th>
                            <th className="text-right p-2 text-[#888888]">Fonte</th>
                          </tr>
                        </thead>
                        <tbody>
                          {measurements.slice(0, 10).map((m, idx) => (
                            <tr key={m.measurement_id || idx} className="border-b border-[#2a2a2a]/50">
                              ...
                              <td className="text-right p-2 text-xs text-[#555555]">{m.source}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
              ...
              <div className="space-y-4">
                {workouts.length === 0 ? (
                  <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-8 text-center">
                    ...
                    <p className="text-[#888888]">Nenhum treino no histórico</p>
                  </Card>
                ) : (
              ...
              <div className="space-y-3">
                {savedInsights.length === 0 ? (
                  <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-8 text-center">
                    ...
                    <p className="text-[#888888]">Nenhuma sugestão salva ainda</p>
                    <p className="text-xs text-[#555555]">Gere sugestões de treino com IA e salve para consultar depois</p>
                  </Card>
                ) : savedInsights.map(insight => (
                  <Card key={insight.insight_id} className="bg-[#0d0d0d] border-[#2a2a2a] p-4">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <p className="text-sm font-medium">{insight.title}</p>
                        <p className="text-[10px] text-[#555555]">{new Date(insight.created_at).toLocaleDateString('pt-BR')}</p>
                      </div>
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-[#555555] hover:text-red-400" onClick={() => handleDeleteInsight(insight.insight_id)}><Trash2 className="w-3 h-3" /></Button>
                    </div>
                    <p className="text-xs text-[#888888] whitespace-pre-wrap">{insight.content}</p>
                  </Card>
                ))}
              </div>
            </TabsContent>
            ...
            <TabsContent value="session">
              {activeSession ? (
                <div className="space-y-4">
                  {/* Session Header */}
                  <Card className="bg-gradient-to-r from-[#0d0d0d] to-[#0a1a0a] border-green-900 p-6">
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h2 className="font-heading text-2xl text-green-400">{activeSession.plan_name}</h2>
                        <p className="text-sm text-[#888888]">Sessão ativa</p>
                      </div>
                      <div className="text-right">
                        <div className="font-data text-4xl text-[#00c896]">{formatTime(sessionElapsed)}</div>
                        <p className="text-xs text-[#888888]">Tempo total</p>
                      </div>
                    </div>
                      ...
                      return (
                        <div>
                          ...
                          <div className="h-3 bg-[#1a1a1a] rounded-full overflow-hidden">
                            <div 
                              className="h-full bg-gradient-to-r from-green-500 to-[#00c896] transition-all duration-500"
                              style={{ width: `${percent}%` }}
                            />
                          </div>
                        </div>
                  ...
                  {/* Rest Timer */}
                  {isResting && (
                    <Card className="bg-[#0d0d0d] border-[#F59E0B] p-6 text-center animate-pulse">
                  ...
                  <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4">
                    <div className="flex items-center justify-between">
                      <Label className="text-xs uppercase tracking-wider text-[#888888]">Timer de Descanso</Label>
                          ...
                          <button
                            ...
                            className={`px-3 py-1 text-xs rounded ${
                              restDuration === sec && !isResting
                                ...
                                : 'bg-[#1a1a1a] border border-[#2a2a2a] text-[#888888] hover:border-[#F59E0B]'
                            }`}
                          >
                            {sec}s
                          </button>
                  ...
                  <div className="space-y-2">
                    {(activeSession.exercises || []).map((ex, idx) => {
                      ...
                      return (
                        <Card key={idx} className={`border-[#2a2a2a] p-0 overflow-hidden ${
                          ex.completed ? 'bg-[#0a1a0a] border-green-900' : 'bg-[#0d0d0d]'
                        }`}>
                          <div className="p-4">
                            <div className="flex items-center gap-3">
                              <div 
                                ...
                                className={`w-8 h-8 rounded-full border-2 flex items-center justify-center cursor-pointer transition-all ${
                                  ex.completed 
                                    ...
                                    : 'border-[#555555] hover:border-[#00c896]'
                                }`}
                              >
                                {ex.completed ? <Check className="w-4 h-4 text-white" /> : <span className="text-xs text-[#555555]">{idx + 1}</span>}
                              </div>
                              ...
                              <div className="flex-1">
                                ...
                                <p className="text-xs text-[#888888]">
                                  ...
                                  {ex.muscle_group && <span className="ml-2 text-[#555555]">· {ex.muscle_group}</span>}
                                </p>
                              </div>
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                {!ex.completed && (
                                  <div className="flex items-center gap-1">
                                    <span className="text-xs text-[#888888]">{setsProgress}/{ex.sets}</span>
                                    <Button 
                                      variant="outline" size="sm"
                                      onClick={() => handleIncrementSets(idx)}
                                      className="h-7 px-2 text-xs border-[#00c896] text-[#00c896] hover:bg-[#00c896] hover:text-black"
                                    >
                                      +1 série
                                    </Button>
                                  </div>
                                ...
                                {Array.from({ length: ex.sets }).map((_, sIdx) => (
                                  <div 
                                    ...
                                    className={`w-3 h-3 rounded-full transition-all ${
                                      sIdx < setsProgress ? 'bg-[#00c896]' : 'bg-[#2a2a2a]'
                                    }`}
                                  />
                                ))}
                              </div>
                          ...
                          {/* Tutorial */}
                          {hasTutorial && expandedTutorials[tutorialKey] && (
                            <div className="bg-[#0a0a1a] border-t border-[#2a2a2a] p-4 space-y-2">
                              {ex.tutorial && (
                                <div>
                                  <p className="text-xs text-[#A855F7] uppercase font-medium mb-1 flex items-center gap-1">
                                    <BookOpenCheck className="w-3 h-3" /> Como executar
                  ...
                  <div className="flex gap-3">
                    ...
                    <Button 
                      ...
                      className="flex-1 bg-gradient-to-r from-green-600 to-[#00c896] text-white hover:opacity-90"
                    >
                ...
                <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-8 text-center">
                  <Dumbbell className="w-12 h-12 text-[#555555] mx-auto mb-4" />
                  <p className="text-[#888888]">Nenhuma sessão ativa</p>
                  <p className="text-sm text-[#555555] mt-2">Vá até a aba "Fichas" e clique em "Iniciar Treino"</p>
                </Card>
              )}
            </TabsContent>
          ...
          <Dialog open={showFeedbackDialog} onOpenChange={setShowFeedbackDialog}>
            <DialogContent className="bg-[#0d0d0d] border-[#2a2a2a] text-white max-w-md">
              ...
              <div className="space-y-6 mt-4">
                ...
                {activeSession && (
                  <div className="grid grid-cols-3 gap-3 text-center">
                    <div className="bg-[#1a1a1a] rounded-lg p-3">
                      <Clock className="w-5 h-5 text-[#00c896] mx-auto mb-1" />
                      <p className="font-data text-xl text-[#00c896]">{formatTime(sessionElapsed)}</p>
                      <p className="text-[10px] text-[#555555] uppercase">Duração</p>
                    </div>
                    <div className="bg-[#1a1a1a] rounded-lg p-3">
                      ...
                      <p className="text-[10px] text-[#555555] uppercase">Exercícios</p>
                    </div>
                    <div className="bg-[#1a1a1a] rounded-lg p-3">
                      ...
                      <p className="text-[10px] text-[#555555] uppercase">Cal (est.)</p>
                    </div>
                  </div>
                )}
                      ...
                      <button
                        ...
                        className={`px-4 py-2 rounded-lg text-sm transition-all ${
                          feedbackData.feeling === f.value
                            ? 'bg-[#00c896] text-black font-medium'
                            : 'bg-[#1a1a1a] border border-[#2a2a2a] text-[#888888] hover:border-[#00c896]'
                        }`}
                      >
                        {f.emoji} {f.label}
                ...
                <div className="flex gap-2">
                  <Button onClick={() => setShowFeedbackDialog(false)} variant="outline" className="flex-1 border-[#2a2a2a]">
                    Voltar
                  </Button>
                  <Button onClick={handleCompleteSession} className="flex-1 bg-gradient-to-r from-green-600 to-[#00c896] text-white">
          ...
          <Dialog open={showImportDialog} onOpenChange={setShowImportDialog}>
            <DialogContent className="bg-[#0d0d0d] border-[#2a2a2a] max-w-md">
              ...
              <div className="space-y-4 py-2">
                <div className={`border-2 border-dashed rounded-lg p-6 text-center ${importFile ? 'border-purple-500 bg-purple-500/10' : 'border-[#2a2a2a]'}`}>
                  {importFile ? (
                    ...
                    <label className="cursor-pointer">
                      ...
                      <p className="text-sm text-[#888888]">Clique para selecionar</p>
                      <p className="text-xs text-[#555555]">PDF ou Imagem da ficha de treino</p>
