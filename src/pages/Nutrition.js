    ...
    <div className="min-h-screen bg-[#050505] text-white flex">
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-8 pb-24 md:pb-8 pt-[72px] md:pt-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-heading text-[#00c896]">Alimentação</h1>
            <p className="text-[#888888]">Controle sua nutrição e alcance seus objetivos</p>
          </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="bg-[#1a1a1a] border border-[#2a2a2a] overflow-x-auto flex-nowrap w-full justify-start md:justify-center">
          ...
          <TabsContent value="overview" className="space-y-6">
                ...
                <CardContent className="space-y-3">
                  <Dialog open={showMealDialog} onOpenChange={setShowMealDialog}>
                    <DialogTrigger asChild>
                      <Button className="w-full bg-[#00c896] hover:bg-[#0066CC]">
                  ...
                  <Dialog open={showGoalsDialog} onOpenChange={setShowGoalsDialog}>
                    ...
                    <DialogContent className="bg-[#0d0d0d] border-[#2a2a2a]">
                      ...
                      <div className="space-y-4 py-4">
                        ...
                        <Button onClick={handleUpdateGoals} className="w-full bg-[#00c896]">
                          Salvar Metas
                        </Button>
                      </div>
                    </DialogContent>
            ...
            <Card className="bg-[#0d0d0d] border-[#2a2a2a]">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <UtensilsCrossed className="w-5 h-5 text-[#00c896]" />
                ...
                {meals.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {meals.map(meal => {
                      ...
                      return (
                        <div key={meal.meal_id} className="bg-[#1a1a1a] p-4 rounded-lg">
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              <MealIcon className="w-4 h-4 text-[#00c896]" />
                          ...
                          <div className="grid grid-cols-4 gap-2 text-xs text-[#888888]">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <Card className="bg-[#0d0d0d] border-[#2a2a2a]">
                  ...
                  <CardContent>
                    ...
                    {weeklyTrend.goals && (
                      <div className="flex justify-between text-[10px] text-[#888888] mt-2 px-2">
                ...
                <Card className="bg-[#0d0d0d] border-[#2a2a2a]">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm flex items-center gap-2"><BarChart3 className="w-4 h-4 text-[#00c896]" />Macronutrientes - Semana</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <ResponsiveContainer width="100%" height={200}>
                      <LineChart data={weeklyTrend.daily}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#2a2a2a" />
                        <XAxis dataKey="day" tick={{ fill: '#888888', fontSize: 11 }} />
                        <YAxis tick={{ fill: '#888888', fontSize: 10 }} unit="g" />
                        <Tooltip contentStyle={{ backgroundColor: '#0d0d0d', border: '1px solid #2a2a2a', color: '#fff', fontSize: 11 }} formatter={(v) => `${v}g`} />
                  ...
                  <CardContent>
                    <ResponsiveContainer width="100%" height={180}>
                      <BarChart data={weeklyTrend.daily}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#2a2a2a" />
                        <XAxis dataKey="day" tick={{ fill: '#888888', fontSize: 11 }} />
                        <YAxis tick={{ fill: '#888888', fontSize: 10 }} unit="ml" />
                        <Tooltip contentStyle={{ backgroundColor: '#0d0d0d', border: '1px solid #2a2a2a', color: '#fff', fontSize: 11 }} formatter={(v) => `${v}ml`} />
                        <Bar dataKey="agua_ml" fill="#3B82F6" name="Água (ml)" radius={[3, 3, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                    {weeklyTrend.goals && (
                      <p className="text-[10px] text-[#888888] text-center mt-1">Meta: {weeklyTrend.goals.water_goal_ml}ml/dia</p>
                    )}
                  </CardContent>
                </Card>
          ...
          <TabsContent value="meals" className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              <Button onClick={() => setShowMealDialog(true)} className="bg-[#00c896]">
                <Plus className="w-4 h-4 mr-2" />
                Nova Refeição
              </Button>
            </div>
            ...
            {["breakfast", "lunch", "dinner", "snack"].map(mealType => {
              ...
              return (
                <Card key={mealType} className="bg-[#0d0d0d] border-[#2a2a2a]">
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                  ...
                  <CardContent>
                    {typeMeals.length === 0 ? (
                      <p className="text-[#888888] text-center py-4">Nenhuma refeição registrada</p>
                    ) : (
                          ...
                          <div key={meal.meal_id} className="bg-[#1a1a1a] p-4 rounded-lg">
                            ...
                            {meal.foods?.length > 0 && (
                              <div className="border-t border-[#2a2a2a] pt-3 mt-3">
                                <span className="text-xs text-[#888888]">Alimentos:</span>
            ...
            {recipes.length === 0 ? (
              <Card className="bg-[#0d0d0d] border-[#2a2a2a]">
                <CardContent className="text-center py-12">
                  <ChefHat className="w-12 h-12 mx-auto text-[#888888] mb-4" />
                  <h3 className="text-lg font-medium mb-2">Nenhuma receita salva</h3>
                  <p className="text-[#888888] mb-4">Use a IA para gerar receitas personalizadas!</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {recipes.map(recipe => (
                  <Card key={recipe.recipe_id} className="bg-[#0d0d0d] border-[#2a2a2a] cursor-pointer hover:border-[#333333] transition-colors" onClick={() => { setSelectedRecipe(recipe); setShowRecipeDetailDialog(true); }}>
                    <CardHeader>
                      ...
                      <div className="space-y-6 py-4">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          <div className="bg-red-500/10 p-3 rounded-lg text-center">
                            ...
                            <p className="text-xs text-[#888888]">Proteínas</p>
                          </div>
                          <div className="bg-yellow-500/10 p-3 rounded-lg text-center">
                            ...
                            <p className="text-xs text-[#888888]">Carboidratos</p>
                          </div>
                          <div className="bg-purple-500/10 p-3 rounded-lg text-center">
                            ...
                            <p className="text-xs text-[#888888]">Gorduras</p>
                          </div>
                        </div>
                          ...
                          <div>
                            ...
                            <div className="space-y-3">
                              {selectedRecipe.instructions.map((step, idx) => (
                                <div key={idx} className="flex items-start gap-3 bg-[#1a1a1a] p-3 rounded-lg">
                        ...
                        {selectedRecipe.tips && (Array.isArray(selectedRecipe.tips) ? selectedRecipe.tips.length > 0 : selectedRecipe.tips.length > 0) && (
                          <div>
                            <h3 className="font-bold text-base mb-3 flex items-center gap-2">
                              <Sparkles className="w-4 h-4 text-[#00c896]" /> Dicas
                            </h3>
          ...
          <TabsContent value="meal_plans" className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2">
                ...
                <Dialog open={showImportDialog} onOpenChange={setShowImportDialog}>
                  <DialogTrigger asChild>
                    <Button data-testid="import-meal-plan-btn" variant="outline" className="border-[#2a2a2a] text-white hover:bg-[#1A1A2E]">
                      <Upload className="w-4 h-4 mr-2" /> Importar Plano
                    </Button>
                  </DialogTrigger>
                  <DialogContent className="bg-[#0d0d0d] border-[#2a2a2a] text-white max-w-md">
                    <DialogHeader>
                      <DialogTitle className="flex items-center gap-2"><Upload className="w-5 h-5 text-[#00c896]" /> Importar Plano Alimentar</DialogTitle>
                      <DialogDescription className="text-[#888888]">Envie um PDF ou foto do seu plano alimentar. A IA vai extrair as refeições automaticamente.</DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 mt-4">
                      <div 
                        ...
                        className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-colors ${
                          importFile ? 'border-[#00c896] bg-[#00c896]/5' : 'border-[#2a2a2a] hover:border-[#A855F7]'
                        }`}
                      >
                        <input
                        ...
                        {importFile ? (
                          <div>
                            <CheckSquare className="w-8 h-8 mx-auto text-[#00c896] mb-2" />
                            <p className="text-sm font-medium text-[#00c896]">{importFile.name}</p>
                            <p className="text-xs text-[#888888] mt-1">{(importFile.size / 1024).toFixed(0)} KB</p>
                          ...
                          <div>
                            ...
                            <p className="text-xs text-[#555555] mt-1">PDF, JPG, PNG ou WEBP</p>
                          </div>
                        )}
                      </div>
                      ...
                      <Button 
                        ...
                        className="w-full bg-gradient-to-r from-[#00c896] to-[#00c896] text-white"
                      >
                ...
                <Dialog open={showMealPlanDialog} onOpenChange={setShowMealPlanDialog}>
                  <DialogTrigger asChild>
                    <Button className="bg-gradient-to-r from-[#A855F7] to-[#00c896] text-white">
                ...
                <DialogContent className="bg-[#0d0d0d] border-[#2a2a2a] text-white max-w-lg max-h-[90vh] overflow-y-auto">
                  ...
                  <div className="space-y-4 mt-4">
                      ...
                      <div className="flex gap-2 mt-1">
                        {[3, 4, 5, 6].map(n => (
                          <button key={n} onClick={() => setMealPlanForm({...mealPlanForm, meals_per_day: n})}
                            className={`flex-1 py-2 rounded text-sm ${mealPlanForm.meals_per_day === n ? 'bg-[#00c896] text-white' : 'bg-[#1a1a1a] text-[#888888] border border-[#2a2a2a]'}`}>
                            {n}
                          </button>
                        ))}
                      </div>
                    ...
                    <div>
                      ...
                      <Input type="number" value={mealPlanForm.calories_target} onChange={(e) => setMealPlanForm({...mealPlanForm, calories_target: parseInt(e.target.value) || 0})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white mt-1" />
                    </div>
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {["vegetariano", "vegano", "sem_gluten", "sem_lactose", "low_carb"].map(r => (
                          <button key={r} onClick={() => toggleMealPlanRestriction(r)}
                            className={`px-3 py-1.5 rounded-full text-xs ${mealPlanForm.restrictions.includes(r) ? 'bg-[#A855F7] text-white' : 'bg-[#1a1a1a] border border-[#2a2a2a] text-[#888888]'}`}>
                            {r.replace('_', ' ')}
                          </button>
                        ))}
                      </div>
                    </div>
                    <Button onClick={handleGenerateMealPlan} disabled={generatingMealPlan} className="w-full bg-gradient-to-r from-[#A855F7] to-[#00c896] text-white">
            ...
            {mealPlans.length === 0 && !selectedMealPlan ? (
              <Card className="bg-[#0d0d0d] border-[#2a2a2a]"><CardContent className="text-center py-12"><Apple className="w-12 h-12 mx-auto text-[#888888] mb-4" /><h3 className="text-lg font-medium mb-2">Nenhum plano alimentar</h3><p className="text-[#888888]">Gere um plano personalizado com IA</p></CardContent></Card>
            ) : (
                ...
                <div className="flex gap-2 overflow-x-auto pb-2">
                  {(selectedMealPlan ? [selectedMealPlan, ...mealPlans.filter(p => p.plan_id !== selectedMealPlan.plan_id)] : mealPlans).map(plan => (
                    <button key={plan.plan_id} onClick={() => setSelectedMealPlan(plan)}
                      className={`px-4 py-2 rounded-lg text-sm whitespace-nowrap ${selectedMealPlan?.plan_id === plan.plan_id ? 'bg-[#00c896] text-white' : 'bg-[#1a1a1a] border border-[#2a2a2a] text-[#888888]'}`}>
                      {plan.name || "Plano"}
                    </button>
                  ))}
                </div>
                {selectedMealPlan && (
                  <div className="space-y-4">
                    <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-6">
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-sm text-[#888888]">{selectedMealPlan.description}</p>
                        ...
                        <div className="flex gap-2">
                          <Button variant="outline" size="sm" onClick={() => handleGenerateShoppingList(selectedMealPlan.plan_id)} className="border-[#2a2a2a] text-xs"><ShoppingCart className="w-3 h-3 mr-1" /> Lista</Button>
                    ...
                    {(selectedMealPlan.days || []).map((day, dIdx) => (
                      <Card key={dIdx} className="bg-[#0d0d0d] border-[#2a2a2a] p-4">
                        <h4 className="font-bold text-sm uppercase text-[#00c896] mb-3">{day.day_label || `Dia ${dIdx + 1}`}</h4>
                        <div className="space-y-3">
                          {(day.meals || []).map((meal, mIdx) => (
                            <div key={mIdx} className="bg-[#1a1a1a] rounded-lg p-3">
                              ...
                              {meal.foods && meal.foods.map((f, fIdx) => (
                                <div key={fIdx} className="flex justify-between text-xs text-[#888888]"><span>{f.name} ({f.quantity})</span><span>{f.calories} kcal</span></div>
                              ))}
                              {meal.preparation && <p className="text-xs text-[#555555] mt-2 italic">{meal.preparation}</p>}
                            </div>
                          ))}
                        </div>
                      </Card>
                    ...
                    {selectedMealPlan.tips && selectedMealPlan.tips.length > 0 && (
                      <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4">
                        ...
                        <ul className="space-y-1">{selectedMealPlan.tips.map((tip, i) => (<li key={i} className="text-xs text-[#888888]">• {tip}</li>))}</ul>
                      </Card>
                    )}
                        ...
                        <div className="space-y-1">
                          {shoppingLists[0].items?.map((item, idx) => (
                            <div key={idx} onClick={() => toggleShoppingItem(shoppingLists[0].list_id, idx)}
                              className={`flex items-center gap-2 p-2 rounded cursor-pointer text-sm ${item.checked ? 'text-[#555555] line-through' : 'text-white'} hover:bg-[#1a1a1a]`}>
                              {item.checked ? <CheckSquare className="w-4 h-4 text-green-500" /> : <div className="w-4 h-4 border border-[#555555] rounded" />}
                              <span className="flex-1">{item.name}</span><span className="text-xs text-[#555555]">{item.quantity}</span>
                            </div>
                          ))}
                        </div>
                      </Card>
          ...
          <TabsContent value="calculator" className="space-y-6">
            <h2 className="text-xl font-bold flex items-center gap-2"><Calculator className="w-5 h-5 text-[#00c896]" /> Calculadora de Saúde</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-6">
                <h3 className="font-bold mb-4">Seus Dados</h3>
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div><Label className="text-xs">Peso (kg)</Label><Input type="number" value={calcForm.weight} onChange={(e) => setCalcForm({...calcForm, weight: parseFloat(e.target.value) || 0})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white mt-1" /></div>
                    <div><Label className="text-xs">Altura (cm)</Label><Input type="number" value={calcForm.height} onChange={(e) => setCalcForm({...calcForm, height: parseFloat(e.target.value) || 0})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white mt-1" /></div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div><Label className="text-xs">Idade</Label><Input type="number" value={calcForm.age} onChange={(e) => setCalcForm({...calcForm, age: parseInt(e.target.value) || 0})} className="bg-[#1a1a1a] border-[#2a2a2a] text-white mt-1" /></div>
                    <div><Label className="text-xs">Sexo</Label>
                      <Select value={calcForm.gender} onValueChange={(v) => setCalcForm({...calcForm, gender: v})}>
                        <SelectTrigger className="bg-[#1a1a1a] border-[#2a2a2a] text-white mt-1"><SelectValue /></SelectTrigger>
                        <SelectContent className="bg-[#1a1a1a] border-[#2a2a2a] text-white"><SelectItem value="male">Masculino</SelectItem><SelectItem value="female">Feminino</SelectItem></SelectContent>
                      </Select>
                    </div>
                  </div>
                  <div><Label className="text-xs">Nível de Atividade</Label>
                    <Select value={calcForm.activity_level} onValueChange={(v) => setCalcForm({...calcForm, activity_level: v})}>
                      <SelectTrigger className="bg-[#1a1a1a] border-[#2a2a2a] text-white mt-1"><SelectValue /></SelectTrigger>
                      <SelectContent className="bg-[#1a1a1a] border-[#2a2a2a] text-white"><SelectItem value="sedentary">Sedentário</SelectItem><SelectItem value="light">Leve (1-3x/sem)</SelectItem><SelectItem value="moderate">Moderado (3-5x/sem)</SelectItem><SelectItem value="active">Ativo (6-7x/sem)</SelectItem><SelectItem value="very_active">Muito ativo</SelectItem></SelectContent>
                    </Select>
                  </div>
                  <div><Label className="text-xs">Objetivo</Label>
                    <Select value={calcForm.objective} onValueChange={(v) => setCalcForm({...calcForm, objective: v})}>
                      <SelectTrigger className="bg-[#1a1a1a] border-[#2a2a2a] text-white mt-1"><SelectValue /></SelectTrigger>
                      <SelectContent className="bg-[#1a1a1a] border-[#2a2a2a] text-white"><SelectItem value="lose">Emagrecer</SelectItem><SelectItem value="maintain">Manter</SelectItem><SelectItem value="gain">Ganhar massa</SelectItem></SelectContent>
                    </Select>
                  </div>
                  <Button onClick={handleCalculateHealth} className="w-full bg-[#00c896] text-black hover:bg-[#00D4E5]"><Calculator className="w-4 h-4 mr-2" /> Calcular</Button>
                </div>
              </Card>
              <div className="space-y-4">
                {calcResult ? (
                  <>
                    <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-6">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="bg-[#1a1a1a] rounded-lg p-4 text-center"><Scale className="w-6 h-6 mx-auto mb-2 text-[#00c896]" /><p className="font-data text-2xl">{calcResult.bmi}</p><p className="text-xs text-[#888888]">IMC</p><p className={`text-xs mt-1 ${calcResult.bmi >= 18.5 && calcResult.bmi < 25 ? 'text-[#00c896]' : 'text-[#FF9500]'}`}>{calcResult.bmi_class}</p></div>
                        <div className="bg-[#1a1a1a] rounded-lg p-4 text-center"><Flame className="w-6 h-6 mx-auto mb-2 text-[#FF9500]" /><p className="font-data text-2xl">{calcResult.bmr}</p><p className="text-xs text-[#888888]">TMB (kcal)</p></div>
                        <div className="bg-[#1a1a1a] rounded-lg p-4 text-center"><Activity className="w-6 h-6 mx-auto mb-2 text-[#A855F7]" /><p className="font-data text-2xl">{calcResult.tdee}</p><p className="text-xs text-[#888888]">GET (kcal)</p></div>
                        <div className="bg-[#1a1a1a] rounded-lg p-4 text-center"><Target className="w-6 h-6 mx-auto mb-2 text-[#00c896]" /><p className="font-data text-2xl text-[#00c896]">{calcResult.calories_target}</p><p className="text-xs text-[#888888]">Meta (kcal)</p></div>
                      </div>
                    </Card>
                    <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-6">
                      <h3 className="font-bold mb-4">Macros Recomendados</h3>
                      <div className="grid grid-cols-3 gap-3">
                        <div className="bg-[#1a1a1a] rounded-lg p-3 text-center"><p className="font-data text-xl text-[#FF6B6B]">{calcResult.macros.protein_g}g</p><p className="text-xs text-[#888888]">Proteína ({calcResult.macros.protein_pct}%)</p></div>
                        <div className="bg-[#1a1a1a] rounded-lg p-3 text-center"><p className="font-data text-xl text-[#FFD700]">{calcResult.macros.carbs_g}g</p><p className="text-xs text-[#888888]">Carboidratos ({calcResult.macros.carbs_pct}%)</p></div>
                        <div className="bg-[#1a1a1a] rounded-lg p-3 text-center"><p className="font-data text-xl text-[#00B4D8]">{calcResult.macros.fat_g}g</p><p className="text-xs text-[#888888]">Gordura ({calcResult.macros.fat_pct}%)</p></div>
                      </div>
                    </Card>
                    <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-4">
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div className="flex justify-between"><span className="text-[#888888]">Peso ideal:</span><span className="font-data">{calcResult.ideal_weight.min}-{calcResult.ideal_weight.max} kg</span></div>
                        <div className="flex justify-between"><span className="text-[#888888]">Água/dia:</span><span className="font-data text-[#00B4D8]">{calcResult.water_liters}L</span></div>
                      </div>
                    </Card>
                  </>
                ) : (
                  <Card className="bg-[#0d0d0d] border-[#2a2a2a]"><CardContent className="text-center py-12"><Calculator className="w-12 h-12 mx-auto text-[#888888] mb-4" /><h3 className="text-lg font-medium mb-2">Calculadora de Saúde</h3><p className="text-[#888888] text-sm">Preencha seus dados para ver IMC, TMB, GET e macros</p></CardContent></Card>
                )}
              </div>
            </div>
          </TabsContent>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="border border-[#2a2a2a] rounded-lg p-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                  <div>
                    <Label className="text-xs text-[#888888] mb-1 block">Alimento</Label>
                    <Input
                  ...
                  <div>
                    <Label className="text-xs text-[#888888] mb-1 block">Quantidade / Peso</Label>
                    <Input
              ...
              {mealForm.foods.length > 0 && (
                <div className="space-y-2">
                  ...
                  {mealForm.foods.map((food, idx) => (
                    <div key={idx} className={`flex items-center justify-between p-3 rounded-lg border ${food.estimated ? 'bg-[#1a1a1a] border-green-500/30' : 'bg-[#1a1a1a] border-[#2a2a2a]'}`}>
                      <div className="flex-1">
                        <span className="font-medium">{food.name}</span>
                        {food.weight && <span className="text-xs text-[#555555] ml-2">({food.weight})</span>}
                        {food.estimated ? (
                          <div className="flex gap-3 mt-1 text-xs text-[#888888]">
                          ...
                          <div className="text-xs text-[#888888] mt-1 italic">Nutrientes pendentes</div>
                        )}
                      </div>
                  ...
                  <Button 
                    ...
                    className="w-full bg-gradient-to-r from-[#A855F7] to-[#00c896] hover:opacity-90 text-white"
                  >
                    {estimatingFood ? (
                  ...
                  {mealForm.foods.every(f => f.estimated) && mealForm.foods.length > 0 && (
                    <div className="bg-[#1a1a1a] border border-green-500/30 rounded-lg p-3">
              ...
              <Button onClick={handleCreateMeal} className="w-full bg-[#00c896]" disabled={mealForm.foods.length === 0 || !mealForm.foods.every(f => f.estimated || f.calories > 0)}>
                Salvar Refeição
              </Button>
            </div>
          </DialogContent>
              ...
              {!suggestedRecipe ? (
                <>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Select value={recipePreferences.meal_type || "any"} onValueChange={(v) => setRecipePreferences({...recipePreferences, meal_type: v === "any" ? "" : v})}>
                        <SelectTrigger className="bg-[#1a1a1a] border-[#2a2a2a]"><SelectValue placeholder="Qualquer" /></SelectTrigger>
                        <SelectContent>
                    ...
                    <div>
                      ...
                      <Select value={recipePreferences.diet_type || "any"} onValueChange={(v) => setRecipePreferences({...recipePreferences, diet_type: v === "any" ? "" : v})}>
                        <SelectTrigger className="bg-[#1a1a1a] border-[#2a2a2a]"><SelectValue placeholder="Balanceada" /></SelectTrigger>
                        <SelectContent>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Input type="number" value={recipePreferences.max_prep_time_minutes} onChange={(e) => setRecipePreferences({...recipePreferences, max_prep_time_minutes: Number(e.target.value)})} className="bg-[#1a1a1a] border-[#2a2a2a]" />
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="bg-[#1a1a1a] p-4 rounded-lg">
                    <h3 className="text-xl font-bold text-[#00c896] mb-2">{suggestedRecipe.name}</h3>
                    <p className="text-[#888888] mb-4">{suggestedRecipe.description}</p>
                    <div className="grid grid-cols-4 gap-2 mb-4">
                      <div className="text-center p-2 bg-[#0d0d0d] rounded"><Flame className="w-4 h-4 mx-auto text-orange-500" /><span className="text-sm">{suggestedRecipe.calories_per_serving} kcal</span></div>
                      <div className="text-center p-2 bg-[#0d0d0d] rounded"><Dumbbell className="w-4 h-4 mx-auto text-red-500" /><span className="text-sm">{suggestedRecipe.protein_per_serving}g P</span></div>
                      <div className="text-center p-2 bg-[#0d0d0d] rounded"><Zap className="w-4 h-4 mx-auto text-yellow-500" /><span className="text-sm">{suggestedRecipe.carbs_per_serving}g C</span></div>
                      <div className="text-center p-2 bg-[#0d0d0d] rounded"><Droplet className="w-4 h-4 mx-auto text-purple-500" /><span className="text-sm">{suggestedRecipe.fat_per_serving}g G</span></div>
                    </div>
                    <div className="mb-4"><h4 className="font-medium mb-2">Ingredientes</h4><ul className="list-disc list-inside space-y-1 text-[#888888]">{suggestedRecipe.ingredients?.map((ing, idx) => <li key={idx}>{ing.quantity} {ing.unit} de {ing.name}</li>)}</ul></div>
                    <div className="mb-4"><h4 className="font-medium mb-2">Modo de Preparo</h4><ol className="list-decimal list-inside space-y-2 text-[#888888]">{suggestedRecipe.instructions?.map((step, idx) => <li key={idx}>{step}</li>)}</ol></div>
                    {suggestedRecipe.tips && <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-3"><h4 className="font-medium text-green-400 mb-1">💡 Dica</h4><p className="text-sm text-[#888888]">{typeof suggestedRecipe.tips === 'string' ? suggestedRecipe.tips : JSON.stringify(suggestedRecipe.tips)}</p></div>}
                  </div>
