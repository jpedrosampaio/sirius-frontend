  ...
  return (
    <div className="flex min-h-screen bg-[#050505]">
      ...
      <div className="flex-1 ml-0 md:ml-64 flex flex-col h-screen">
        <div className="p-4 md:p-6 border-b border-[#2a2a2a] pt-[72px] md:pt-6">
          <h1 className="font-heading text-xl md:text-3xl mb-1">ASSISTENTE SIRIUS</h1>
          <p className="text-xs md:text-sm text-[#888888]">Chat integrado: finanças, estudos, treinos, receitas e mais. Peça algo e ele salva no app!</p>
        </div>

        <div ref={scrollRef} className="flex-1 p-4 md:p-6 overflow-y-auto pb-36 md:pb-28">
          <div className="max-w-4xl mx-auto space-y-4">
            {messages.length === 0 ? (
              <Card className="bg-[#0d0d0d] border-[#2a2a2a] p-6 md:p-8 text-center">
                <Sparkles className="w-10 h-10 md:w-12 md:h-12 text-[#00c896] mx-auto mb-4" />
                <p className="text-[#888888] text-sm md:text-base mb-3">Sou seu assistente integrado! Posso ajudar com:</p>
                <div className="grid grid-cols-2 gap-3 max-w-md mx-auto">
                  <div className="bg-[#1a1a1a] p-3 rounded-lg text-left">
                    <ChefHat className="w-5 h-5 text-green-400 mb-1" />
                    <p className="text-xs font-medium">Receitas</p>
                    <p className="text-[10px] text-[#555555]">"Me dê uma receita de frango fitness"</p>
                  </div>
                  <div className="bg-[#1a1a1a] p-3 rounded-lg text-left">
                    <Dumbbell className="w-5 h-5 text-orange-400 mb-1" />
                    <p className="text-xs font-medium">Treinos</p>
                    <p className="text-[10px] text-[#555555]">"Monte um treino de peito e tríceps"</p>
                  </div>
                  <div className="bg-[#1a1a1a] p-3 rounded-lg text-left">
                    <DollarSign className="w-5 h-5 text-blue-400 mb-1" />
                    <p className="text-xs font-medium">Finanças</p>
                    <p className="text-[10px] text-[#555555]">"Como economizar R$ 500/mês?"</p>
                  </div>
                  <div className="bg-[#1a1a1a] p-3 rounded-lg text-left">
                    <BookOpen className="w-5 h-5 text-purple-400 mb-1" />
                    <p className="text-xs font-medium">Estudos</p>
                    <p className="text-[10px] text-[#555555]">"Dicas para estudar direito constitucional"</p>
                  </div>
                </div>
                <p className="text-[10px] text-[#555555] mt-3">💡 Receitas e treinos pedidos aqui são salvos automaticamente no app!</p>
              </Card>
            ) : (
              messages.map((msg, idx) => (
                ...
                >
                  <div className={`flex items-start space-x-2 md:space-x-3 max-w-[90%] md:max-w-2xl ${msg.role === 'user' ? 'flex-row-reverse space-x-reverse' : ''}`}>
                    <div className={`w-7 h-7 md:w-8 md:h-8 rounded-sm flex items-center justify-center flex-shrink-0 ${msg.role === 'user' ? 'bg-[#00c896]' : 'bg-[#222222]'}`}>
                      {msg.role === 'user' ? <User className="w-4 h-4 md:w-5 md:h-5" /> : <Bot className="w-4 h-4 md:w-5 md:h-5" />}
                    </div>
                    <div className={`p-3 md:p-4 rounded-sm ${msg.role === 'user' ? 'bg-[#00c896]/20 border border-[#00c896]/30' : 'bg-[#0d0d0d] border border-[#2a2a2a]'}`}>
                      {msg.role === 'user' ? (
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="flex items-start space-x-3">
                  <div className="w-8 h-8 rounded-sm bg-[#222222] flex items-center justify-center"><Bot className="w-5 h-5" /></div>
                  <div className="bg-[#0d0d0d] border border-[#2a2a2a] p-4 rounded-sm"><Loader2 className="w-4 h-4 animate-spin text-[#00c896]" /></div>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="p-3 md:p-6 border-t border-[#2a2a2a] bg-[#0d0d0d]/95 backdrop-blur-lg fixed bottom-[60px] md:bottom-0 left-0 right-0 md:left-64">
          <form onSubmit={handleSend} className="max-w-4xl mx-auto">
            <div className="flex items-center space-x-2 md:space-x-3">
              <Input value={content} onChange={(e) => setContent(e.target.value)} placeholder="Peça uma receita, treino, dica financeira ou qualquer coisa..." className="flex-1 bg-[#1a1a1a] border-[#2a2a2a] text-white font-mono text-sm" disabled={loading} />
              <Button type="submit" disabled={loading || !content.trim()} className="bg-[#00c896] hover:bg-[#0062CC] px-3 md:px-4">
