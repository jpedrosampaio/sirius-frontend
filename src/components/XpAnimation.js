  ...
  return (
    <div className={`fixed inset-0 pointer-events-none z-[100] flex items-center justify-center transition-opacity duration-300 ${animating ? 'opacity-100' : 'opacity-0'}`}>
      <div className={`transform transition-all duration-700 ${animating ? 'scale-100 translate-y-0' : 'scale-50 translate-y-20'}`}>
        <div className="bg-[#0d0d0d]/90 backdrop-blur-xl border-2 border-[#FFD700] rounded-2xl p-8 text-center shadow-2xl shadow-[#FFD700]/20">
          <div className="relative">
            ...
            <div className="absolute -top-2 -right-6 animate-ping delay-150">
              <Star className="w-3 h-3 text-[#00c896]" />
            </div>
            ...
            <p className="text-[#888888] text-xs mt-2">Continue assim!</p>
          </div>
        </div>
      </div>
    </div>
...
export function RankUpAnimation({ newRank, show, onComplete }) {
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-[100] flex items-center justify-center">
      <div className="bg-[#0d0d0d]/95 backdrop-blur-xl border-2 border-[#A855F7] rounded-2xl p-10 text-center shadow-2xl animate-[bounceIn_0.5s_ease-out]">
        ...
        <p className="text-[#888888] text-sm mt-2">Parabéns pela evolução!</p>
      </div>
    </div>
  );
}
...
export function LoadingSkeleton({ type = "card", count = 3 }) {
  ...
  if (type === "card") {
    return (
      <div className="space-y-4">
        {skeletons.map((_, i) => (
          <div key={i} className="bg-[#0d0d0d] border border-[#2a2a2a] rounded-lg p-6 animate-pulse">
        ...
        {skeletons.map((_, i) => (
          <div key={i} className="bg-[#0d0d0d] border border-[#2a2a2a] rounded-lg p-6 animate-pulse">
            <div className="flex justify-between mb-4">
              ...
              <div className="w-16 h-6 rounded bg-[#2a2a2a]" />
            </div>
            <div className="h-3 bg-[#1a1a1a] rounded w-2/3" />
          </div>
        ))}
  ...
  if (type === "list") {
    return (
      <div className="space-y-2">
        {skeletons.map((_, i) => (
          <div key={i} className="bg-[#0d0d0d] border border-[#2a2a2a] rounded-lg p-4 animate-pulse flex items-center gap-3">
